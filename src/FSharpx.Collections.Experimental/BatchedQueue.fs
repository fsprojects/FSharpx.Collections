...
type BatchedQueue<'T>(front: list<'T>, rBack: list<'T>) =
    ...
    static member private length(q: BatchedQueue<'T>) =
        q.front.Length + q.rBack.Length
    ...
    member this.Rev = BatchedQueue<'T>(rBack, front) |> BatchedQueue.checkf

    ///returns a new queue with the element added to the end
    member this.Snoc x =
        BatchedQueue<'T>(front, x :: rBack) |> BatchedQueue.checkf

    ///returns a new queue of the elements trailing the first element
    member this.Tail =
        match front with
        | hd :: tl -> BatchedQueue<'T>(tl, rBack) |> BatchedQueue.checkf
        | _ -> raise Exceptions.Empty
...
module BatchedQueue =
    ...
    ///returns a new queue with the element added to the end
    let inline snoc (x: 'T) (q: BatchedQueue<'T>) =
        (q.Snoc x)
    ...
    ///returns option queue of the elements trailing the first element
    let inline tryGetTail(q: BatchedQueue<'T>) =
        q.TryGetTail
    ...
    ///returns option first element and tail
    let inline tryUncons(q: BatchedQueue<'T>) =
        q.TryUncons
