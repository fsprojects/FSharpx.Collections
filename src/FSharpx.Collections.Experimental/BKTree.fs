...
type BKTree<'T> =
    ...
    member x.ToList() =
        match x with
        | Empty -> []
        | Node(a, _, m) -> a :: (m |> IntMap.values |> List.collect(fun t -> t.ToList()))
...
module BKTree =
    ...
    let singleton a =
        Node(a, 1, IntMap.empty)

    let rec private add distance a =
        function
    ...
    let rec private exists distance a =
        function
        | Empty -> false
        | Node(b, _, map) ->
            let d = distance a b
    ...
    let rec private existsDistance distance n a =
        function
        | Empty -> false
        | Node(b, _, map) ->
            let d = distance a b
    ...
    let inline toList(tree: _ BKTree) =
        tree.ToList()
    ...
    let rec private toListDistance distance n a =
        function
        | Empty -> []
        | Node(b, _, map) ->
            let d = distance a b
    ...
    let private ofCollection pred distance xs =
        xs |> pred ((fun f a b -> f b a) (add distance)) empty

    let private ofList distance xs =
        ofCollection List.fold distance xs

    let private ofSeq distance xs =
        ofCollection Seq.fold distance xs

    let private ofArray distance xs =
        ofCollection Array.fold distance xs

    let private concat distance xs =
        xs |> List.collect toList |> ofList distance

    let rec private delete distance a =
        function
        | Empty -> Empty
        | Node(b, _, map) ->
            let d = distance a b
            ...
            if d = 0 then
                concat distance (IntMap.values map)
            else
    ...
    type 'T Functions(distance: 'T -> 'T -> int) =
        ...
        member x.exists a tree =
            exists distance a tree

        member x.existsDistance n a tree =
            existsDistance distance n a tree

        member x.toListDistance n a tree =
            toListDistance distance n a tree
        ...
        member x.append t1 t2 =
            x.concat [ t1; t2 ]

        member x.delete a tree =
            delete distance a tree
    ...
    let private byteStringDistance xs =
        function
