...
type Timeseries<'T>(startDate: DateTimeOffset, granularity: TimeSpan, position: int, values: seq<'T>) =
    ...
    let offset (a: DateTimeOffset) (b: DateTimeOffset) =
        (b.Subtract(a).TotalMinutes / granularity.TotalMinutes)
        |> floor
        |> int

    new(startDate, granularity, values) = Timeseries(startDate, granularity, 0, values)

    new(startDate, granularity, size) = Timeseries(startDate, granularity, Array.zeroCreate size)
    ...
    member x.AsTimeseries(?startDate: DateTimeOffset) =
        ...
        x.Buffer.ToArray()
        |> convertToSeries x.StartDate x.Granularity
        |> Array.filter(fun (d, _) -> d >= date)
    ...
    member x.Series = x.Buffer.ToArray() |> convertToSeries x.StartDate x.Granularity
