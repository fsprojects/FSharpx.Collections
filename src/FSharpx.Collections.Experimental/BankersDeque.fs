...
type BankersDeque<'T>(c: int, frontLength: int, front: LazyList<'T>, rBackLength: int, rBack: LazyList<'T>) =
    ...
    static member private length(q: BankersDeque<'T>) =
    ...
    static member internal Empty c =
        new BankersDeque<'T>(c, 0, (LazyList.empty), 0, (LazyList.empty))
    ...
    member this.Rev = (new BankersDeque<'T>(c, rBackLength, rBack, frontLength, front))
...
module BankersDeque =
    ...
    let (|Cons|Nil|)(q: BankersDeque<'T>) =
        match q.TryUncons with
        | Some(a, b) -> Cons(a, b)
        | None -> Nil

    let (|Snoc|Nil|)(q: BankersDeque<'T>) =
    ...
    let append (xs: BankersDeque<'T>) (ys: BankersDeque<'T>) =
        BankersDeque.AppendC stndC xs ys

    ///O(ys-xs). Returns a deque of the two deques concatenated, c is front-back stream ratio constant, should be at least 2.
    let appendC c (xs: BankersDeque<'T>) (ys: BankersDeque<'T>) =
    ...
    ///O(1), amortized. Returns option first element.
    let inline tryGetHead(q: BankersDeque<'T>) =
        q.TryGetHead
    ...
    ///O(1), amortized. Returns option deque of the elements before the last element.
    let inline tryGetInit(q: BankersDeque<'T>) =
        q.TryGetInit
    ...
    ///O(1), amortized. Returns option last element.
    let inline tryGetLast(q: BankersDeque<'T>) =
        q.TryGetLast
    ...
    ///O(n), worst case. Returns element by index.
    let inline lookup i (q: BankersDeque<'T>) =
        q.Lookup i

    ///O(n), worst case. Returns option element by index.
    let inline tryLookup i (q: BankersDeque<'T>) =
        q.TryLookup i
    ...
    let ofCatLists xs ys =
        BankersDeque.OfCatListsC stndC xs ys
    ...
    let ofCatSeqs xs ys =
        BankersDeque.OfCatSeqsC stndC xs ys
    ...
    ///O(n), worst case. Returns deque with element removed by index.
    let inline remove i (q: BankersDeque<'T>) =
        q.Remove i

    ///O(n), worst case. Returns option deque with element removed by index.
    let inline tryRemove i (q: BankersDeque<'T>) =
        q.TryRemove i
    ...
    ///O(1), amortized. Returns a new deque with the element added to the end.
    let inline snoc (x: 'T) (q: BankersDeque<'T>) =
        (q.Snoc x)
    ...
    ///O(1), amortized. Returns option deque of the elements trailing the first element.
    let inline tryGetTail(q: BankersDeque<'T>) =
        q.TryGetTail
    ...
    ///O(1), amortized. Returns option first element and tail.
    let inline tryUncons(q: BankersDeque<'T>) =
        q.TryUncons
    ...
    ///O(1), amortized. Returns option init and the last element.
    let inline tryUnsnoc(q: BankersDeque<'T>) =
        q.TryUnsnoc

    ///O(n), worst case. Returns deque with element updated by index.
    let inline update i y (q: BankersDeque<'T>) =
        q.Update i y

    ///O(n), worst case. Returns option deque with element updated by index.
    let inline tryUpdate i y (q: BankersDeque<'T>) =
        q.TryUpdate i y
