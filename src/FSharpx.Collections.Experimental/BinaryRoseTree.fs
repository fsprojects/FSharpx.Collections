...
type BinaryRoseTree<'T> = //{ Root: 'T; Children: 'T BinaryRoseTree Vector }
    ...
    override x.Equals y =
        match y with
        | :? BinaryRoseTree<'T> as y -> (x :> _ IEquatable).Equals y
        | _ -> false
...
module BinaryRoseTree =
    ...
    let inline createTree root (children: BinaryRoseTree<'T>) =
        Node(root, children, Nil)

    let inline createForest root (children: BinaryRoseTree<'T>) (sibling: BinaryRoseTree<'T>) =
        Node(root, children, sibling)

    let empty = Nil

    let inline singleton root =
        createTree root Nil
    ...
    let private map2 nodeF leafV (x: _ BinaryRoseTree) =
        ...
        let rec loop (x: _ BinaryRoseTree) cont =
            match x with
            | Node(a, children, siblings) -> loop children (fun lacc -> loop siblings (fun racc -> cont(nodeF a lacc racc)))
            | Nil -> cont leafV
