...
type BlockResizeArray<'T>() =
    ...
    let mutable arrays =
        Array.init initArraysCount (fun _ -> Array.zeroCreate<'T> blockSize)
    ...
    let countTop c =
        ...
        if c <> 0 then
            if tail = 0 then blockSize - 1 else tail - 1
        else
            -1
    ...
    member private this.SetActive i =
        active <- i
    ...
    member private this.SetArr i arr =
        arrays.[i] <- arr
            ...
            let newArr =
                Array.init l (fun i ->
                    if i < blocksCount then
                        Array.init blockSize (fun j -> f(blockSize * i + j))
                    else
                        Array.zeroCreate<'T> blockSize)

            if smallPartCount <> 0 then
    ...
    member this.Filter f =
        ...
        for i = 0 to active - 1 do
            for j = 0 to blockSize - 1 do
                ...
                if f x && i * blockSize + j < this.Length then
                    a.[index] <- x
                    index <- index + 1
                    resCount <- resCount + 1
...
module BlockResizeArray =
    ...
    ///Applies the given function to each element of the block resize array.
    let iter (f: 'T -> unit) (bra: BlockResizeArray<_>) =
        bra.Iter f

    ///Applies a function to each element of the collection, threading an accumulator argument through the computation.
    let fold f s (bra: BlockResizeArray<_>) =
        bra.Fold f s

    ///Returns the length of a block resize array.
    let inline count(bra: BlockResizeArray<_>) =
        bra.Length
    ...
    ///Returns a new collection containing only the elements of the collection for which the given predicate returns true.
    let filter (f: 'T -> bool) (bra: BlockResizeArray<_>) =
        bra.Filter f

    ///Returns the first element for which the given function returns true. Return None if no such element exists.
    let tryFind (f: 'T -> bool) (bra: BlockResizeArray<_>) =
        bra.TryFind f

    ///Returns the first element for which the given function returns true.
    let find (f: 'T -> bool) (bra: BlockResizeArray<_>) =
        bra.Find f

    ///Creates an array from the given block resize array.
    let toArray(bra: BlockResizeArray<_>) =
        bra.ToArray()
