...
type BinomialHeap<'T when 'T: comparison>(isDescending: bool, heap: list<BinomialTree<'T>>) =
    ...
    member this.Length() =
        List.fold (fun acc (Node(r, _, _)) -> acc + int(2.0 ** (float r))) 0 this.heap

    static member internal empty isDescending =
        BinomialHeap<'T>(isDescending, [])

    static member private isEmpty: BinomialHeap<'T> -> bool =
        function
    ...
    static member private insertTree (isDescending: bool) (t: BinomialTree<'T>) (h: list<BinomialTree<'T>>) : list<BinomialTree<'T>> =
        ...
        | [] -> [ t ]
        | hd :: tl as t' ->
            if BinomialHeap.rank t < BinomialHeap.rank hd then
                t :: t'
            else
                BinomialHeap.insertTree isDescending (BinomialHeap.link isDescending t hd) tl
    ...
    static member private merge isDescending (h1: list<BinomialTree<'T>>) (h2: list<BinomialTree<'T>>) : list<BinomialTree<'T>> =
        ...
        | x, [] -> x
        | (hd1 :: tl1), (hd2 :: tl2) ->
            if BinomialHeap.rank hd1 < BinomialHeap.rank hd2 then
                hd1 :: BinomialHeap.merge isDescending tl1 h2
            elif BinomialHeap.rank hd1 > BinomialHeap.rank hd2 then
                hd2 :: BinomialHeap.merge isDescending h1 tl2
            else
                BinomialHeap.insertTree isDescending (BinomialHeap.link isDescending hd1 hd2) (BinomialHeap.merge isDescending (tl1) (tl2))

    static member private removeHeadTree: bool * list<BinomialTree<'T>> -> (BinomialTree<'T> * list<BinomialTree<'T>>) =
        function
        ...
        | isDescending, hd :: tl ->
            ...
            if isDescending then
                if BinomialHeap.root hd > BinomialHeap.root hd' then
                    hd, tl
                else
                    hd', hd :: tl'
            else if BinomialHeap.root hd <= BinomialHeap.root hd' then
                hd, tl
            else
                hd', hd :: tl'
    ...
    static member internal ofSeq (descending: bool) (s: seq<'T>) : BinomialHeap<'T> =
        if Seq.isEmpty s then
            ...
            let x = Seq.fold (fun acc elem -> [ Node(0, elem, []) ] :: acc) [] s

            let pairWiseMerge(l: list<list<BinomialTree<'T>>>) =
                let rec loop(acc: list<list<BinomialTree<'T>>>) : list<list<BinomialTree<'T>>> -> list<list<BinomialTree<'T>>> =
                    function
            ...
            let rec loop: list<list<BinomialTree<'T>>> -> list<BinomialTree<'T>> =
                function
                | [ h ] -> h
    ...
    static member private foldHeap nodeF leafV (h: list<BinomialTree<'T>>) =
        ...
        let rec loop (h: list<BinomialTree<'T>>) cont =
            match h with
            | Node(_, a, h') :: tl -> loop h' (fun lacc -> loop tl (fun racc -> cont(nodeF a lacc racc)))
            | _ -> cont leafV

        loop h (fun x -> x)

    static member private inOrder(h: list<BinomialTree<'T>>) =
        (BinomialHeap.foldHeap (fun x l r acc -> l(x :: (r acc))) (fun acc -> acc) h) []

    ///O(log n). Returns the min or max element.
    member this.Head() =
        BinomialHeap.head this.IsDescending this.heap
    ...
    member this.TryGetHead() =
        if this.heap.IsEmpty then
            ...
            Some(BinomialHeap.head this.IsDescending this.heap)

    ///O(log n). Returns a new heap with the element inserted.
    member this.Insert x =
        BinomialHeap<'T>(this.IsDescending, (BinomialHeap.insert this.IsDescending x this.heap))
    ...
    member this.TryMerge(xs: BinomialHeap<'T>) =
        if (this.IsDescending <> xs.IsDescending) then
            ...
            Some(BinomialHeap<'T>(this.IsDescending, (BinomialHeap.merge this.IsDescending this.heap xs.heap)))
    ...
    member this.TryGetTail() =
        if this.heap.IsEmpty then
            ...
            Some(BinomialHeap<'T>(this.IsDescending, (BinomialHeap.tail this.IsDescending this.heap)))

    ///O(log n). Returns the head element and tail.
    member this.Uncons() =
        BinomialHeap.uncons this.IsDescending this.heap
    ...
    member this.TryUncons() =
        if this.heap.IsEmpty then
            ...
            Some(BinomialHeap.uncons this.IsDescending this.heap)
    ...
    interface IReadOnlyCollection<'T> with
        ...
        member this.GetEnumerator() =
            let e =
                ...
                then
                    BinomialHeap.inOrder this.heap |> List.sort |> List.rev |> List.toSeq
                else
                    BinomialHeap.inOrder this.heap |> List.sort |> List.toSeq
    ...
    interface IPriorityQueue<'T> with
        ...
        member this.Insert element =
            this.Insert element :> IPriorityQueue<'T>
...
module BinomialHeap =
    ...
    let (|Cons|Nil|)(h: BinomialHeap<'T>) =
        match h.TryUncons() with
        | Some(a, b) -> Cons(a, b)
        | None -> Nil

    ///O(1). Returns a empty heap.
    let empty(descending: bool) =
        BinomialHeap.empty descending
    ...
    ///O(log n). Returns option first min or max element.
    let inline tryGetHead(xs: BinomialHeap<'T>) =
        xs.TryGetHead()

    ///O(log n). Returns a new heap with the element inserted.
    let inline insert x (xs: BinomialHeap<'T>) =
        xs.Insert x

    ///O(1). Returns true if the heap has no elements.
    let inline isEmpty(xs: BinomialHeap<'T>) =
        xs.IsEmpty

    ///O(1). Returns true if the heap has max element at head.
    let inline isDescending(xs: BinomialHeap<'T>) =
        xs.IsDescending

    ///O(log n). Returns the count of elements.
    let inline length(xs: BinomialHeap<'T>) =
        xs.Length()

    ///O(log n) Returns heap from merging two heaps, both must have same isDescending.
    let inline merge (xs: BinomialHeap<'T>) (ys: BinomialHeap<'T>) =
        xs.Merge ys

    ///O(log n). Returns heap option from merging two heaps.
    let inline tryMerge (xs: BinomialHeap<'T>) (ys: BinomialHeap<'T>) =
        xs.TryMerge ys

    ///O(n * log n). Returns heap from the sequence.
    let ofSeq descending s =
    ...
    ///O(log n). Returns option heap of the elements trailing the head
    let inline tryGetTail(xs: BinomialHeap<'T>) =
        xs.TryGetTail()

    /// O(log n). Returns the head element and tail.
    let inline uncons(xs: BinomialHeap<'T>) =
        xs.Uncons()

    /// O(log n). Returns option head element and tail.
    let inline tryUncons(xs: BinomialHeap<'T>) =
        xs.TryUncons()
