...
type IndexedRoseTree<'T> =
    ...
    override x.Equals y =
        match y with
        | :? IndexedRoseTree<'T> as y -> (x :> IEquatable<_>).Equals y
        | _ -> false

    override x.GetHashCode() =
        391 + (box x.Root).GetHashCode() * 23 + x.Children.GetHashCode()

    interface IEquatable<IndexedRoseTree<'T>> with
        member x.Equals y =
            obj.Equals(x.Root, y.Root)
            && (x.Children :> _ seq).SequenceEqual y.Children
...
module IndexedRoseTree =
    ...
    let rec map f (x: _ IndexedRoseTree) = { IndexedRoseTree.Root = f x.Root
                                             Children = PersistentVector.map (map f) x.Children }

    let rec ap x f = { IndexedRoseTree.Root = f.Root x.Root
                       Children =
