...
type PhysicistQueue<'T>(prefix: list<'T>, frontLength: int, front: Lazy<list<'T>>, rBackLength: int, rBack: list<'T>) =
    ...
    static member private length(q: PhysicistQueue<'T>) =
        q.frontLength + q.rBackLength

    static member internal Empty() =
        PhysicistQueue<'T>([], 0, lazy [], 0, [])

    static member internal fold (f: ('State -> 'T -> 'State)) (state: 'State) (q: PhysicistQueue<'T>) : 'State =
        let s =
            if (q.prefix.Length = q.frontLength) then
                List.fold f state q.prefix
            else
                List.fold f state q.front.Value
    ...
    static member internal foldBack (f: ('T -> 'State -> 'State)) (q: PhysicistQueue<'T>) (state: 'State) : 'State =
        ...
        if (q.prefix.Length = q.frontLength) then
            ...
            (List.foldBack f q.front.Value s)

    static member internal OfList(xs: list<'T>) =
        PhysicistQueue<'T>(xs, xs.Length, (lazy xs), 0, [])
    ...
    member this.Rev =
        if (prefix.Length = frontLength) then
            PhysicistQueue<'T>(rBack, rBackLength, (lazy rBack), frontLength, prefix)
            |> PhysicistQueue.check
        else
            PhysicistQueue<'T>(rBack, rBackLength, (lazy rBack), frontLength, front.Value)
            |> PhysicistQueue.check
...
module PhysicistQueue =
    ...
    let (|Cons|Nil|)(q: PhysicistQueue<'T>) =
    ...
    let fold (f: ('State -> 'T -> 'State)) (state: 'State) (q: PhysicistQueue<'T>) =
        PhysicistQueue<_>.fold f state q
    ...
    let foldBack (f: ('T -> 'State -> 'State)) (q: PhysicistQueue<'T>) (state: 'State) =
        PhysicistQueue<_>.foldBack f q state

    ///O(1), amortized. Returns the first element.
    let inline head(q: PhysicistQueue<'T>) = q.Head

    ///O(1), amortized. Returns option first element.
    let inline tryGetHead(q: PhysicistQueue<'T>) =
        q.TryGetHead
    ...
    ///O(1), amortized. Returns a new queue with the element added to the end.
    let inline snoc (x: 'T) (q: PhysicistQueue<'T>) =
        (q.Snoc x)
    ...
    ///O(1), amortized. Returns option queue of the elements trailing the first element.
    let inline tryGetTail(q: PhysicistQueue<'T>) =
        q.TryGetTail
    ...
    ///O(1), amortized. Returns option first element and tail.
    let inline tryUncons(q: PhysicistQueue<'T>) =
        q.TryUncons
