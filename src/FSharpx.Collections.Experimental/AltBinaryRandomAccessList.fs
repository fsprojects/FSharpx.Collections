...
type AltBinRndAccList<'T> =
    ...
    interface IRandomAccessList<'T> with
        ...
        member this.Cons(x: 'T) =
            AltBinRndAccList.cons x this :> _

        member this.Count() =
            AltBinRndAccList.length(0, 1, this)
        ...
        member this.Lookup i =
            AltBinRndAccList.lookup i this

        member this.Item
            with get i = AltBinRndAccList.lookup i this

        member this.TryLookup i =
            AltBinRndAccList.tryLookup i this

        member this.Rev() =
            AltBinRndAccList.rev this :> _
        ...
        member this.Update i y =
            AltBinRndAccList.fupdate((fun x -> y), i, this) :> _
...
and AltBinRndAccList<'T> with
    ...
    static member internal cons(x: 'T) : AltBinRndAccList<'T> -> AltBinRndAccList<'T> =
        function
        | Nil -> One(x, Nil)
        | Zero ps -> One(x, ps)
        | One(y, ps) -> Zero(AltBinRndAccList.cons (x, y) ps)

    static member internal isEmpty: AltBinRndAccList<'T> -> bool =
        function
        | Nil -> true
        | _ -> false

    static member internal length: int * int * AltBinRndAccList<'T> -> int =
        function
        | len, acc, Nil -> len
    ...
    static member internal lookup(i: int) : AltBinRndAccList<'T> -> 'T =
        function
        ...
        | One(x, ps) ->
            if i = 0 then
                ...
                AltBinRndAccList.lookup (i - 1) (Zero ps)
        | Zero ps ->
            let (x, y) = AltBinRndAccList.lookup (i / 2) ps
            if i % 2 = 0 then x else y

    static member internal tryLookup(i: int) : AltBinRndAccList<'T> -> 'T option =
        function
        | Nil -> None
        | One(x, ps) ->
            if i = 0 then
                ...
                AltBinRndAccList.tryLookup (i - 1) (Zero ps)
        | Zero ps ->
        ...
        if Seq.isEmpty s then
            ...
            let rec loop (acc: AltBinRndAccList<'T>) dec (a': array<'T>) =
                if dec < 0 then
                    ...
                    loop (AltBinRndAccList.cons a'.[dec] acc) (dec - 1) a'

            loop Nil (a.Length - 1) a

    static member internal uncons: AltBinRndAccList<'T> -> 'T * AltBinRndAccList<'T> =
        function
    ...
    static member internal tryUncons: AltBinRndAccList<'T> -> ('T * AltBinRndAccList<'T>) option =
        function
        | Nil -> None
    ...
    static member internal fremove: int * array<'T> * int * AltBinRndAccList<'T> -> int * AltBinRndAccList<'T> =
        function
    ...
    static member internal ftryRemove: int * array<'T> * int * AltBinRndAccList<'T> -> int * AltBinRndAccList<'T> option =
        function
        | i, _, _, Nil -> 0, None
    ...
    static member internal rev: AltBinRndAccList<'T> -> AltBinRndAccList<'T> =
        function
        | Nil -> Nil
        | xs ->
            let rec loop xs' acc =
    ...
    static member internal fupdate: ('T -> 'T) * int * AltBinRndAccList<'T> -> AltBinRndAccList<'T> =
        function
        ...
        | f, i, Zero ps ->
            let f'(x, y) =
                if i % 2 = 0 then f x, y else x, f y

            Zero(AltBinRndAccList.fupdate(f', i / 2, ps))

    static member internal ftryUpdate: ('T -> 'T) * int * AltBinRndAccList<'T> -> AltBinRndAccList<'T> option =
        function
        | f, i, Nil -> None
    ...
    static member internal append: AltBinRndAccList<'T> * AltBinRndAccList<'T> -> AltBinRndAccList<'T> =
        function
        | Nil, Nil -> Nil
        | xs, Nil -> xs
        | Nil, ys -> ys
    ...
    member this.Update i y =
        AltBinRndAccList.fupdate((fun x -> y), i, this)
...
module AltBinaryRandomAccessList =
    ...
    let (|Cons|Nil|)(l: AltBinRndAccList<'T>) =
    ...
    ///O(1). Returns true if the random access list has no elements.
    let inline isEmpty(xs: AltBinRndAccList<'T>) =
        xs.IsEmpty

    ///O(log n). Returns the count of elements.
    let inline length(xs: AltBinRndAccList<'T>) =
        xs.Length()

    ///O(log n). Returns element by index.
    let rec lookup i xs =
        AltBinRndAccList.lookup i xs

    ///O(log n). Returns option element by index.
    let rec tryLookup i xs =
        AltBinRndAccList.tryLookup i xs

    ///O(n). Returns random access list from the sequence.
    let ofSeq s =
        AltBinRndAccList.ofSeq s

    ///O(n). Returns random access list with element removed by index.
    let inline remove i (xs: AltBinRndAccList<'T>) =
        xs.Remove i

    ///O(n). Returns option random access list with element removed by index.
    let inline tryRemove i (xs: AltBinRndAccList<'T>) =
        xs.TryRemove i
    ...
    ///O(log n). Returns the option first element and tail.
    let inline tryUncons(xs: AltBinRndAccList<'T>) =
        xs.TryUncons

    ///O(log n). Returns random access list with element updated by index.
    let inline update i y (xs: AltBinRndAccList<'T>) =
        xs.Update i y

    ///O(log n). Returns option random access list with element updated by index.
    let inline tryUpdate i y (xs: AltBinRndAccList<'T>) =
        xs.TryUpdate i y
