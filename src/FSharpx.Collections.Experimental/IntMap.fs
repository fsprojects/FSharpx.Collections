...
type IntMap<'T> =
    ...
    member x.ToList() =
        x.FoldBackWithKey (fun k x xs -> (k, x) :: xs) []
...
module IntMap =
    ...
    let inline private match' i p m =
        mask i m = p
    ...
    let inline private shorter m1 m2 =
        (uint32 m1) > (uint32 m2)
    ...
    let rec find k m =
        let notFound() =
            failwith
            <| sprintf "IntMap.find: key %d is not an element of the map" k
    ...
    let insertWith f k x t =
        insertWithKey (fun _ x' y' -> f x' y') k x t
    ...
    let adjustWithKey f k m =
        updateWithKey (fun k' x -> Some(f k' x)) k m

    ///O(min(n,W)). Adjust a value at a specific key. When the key is not a member of the map, the original map is returned. Credit: Haskell.org
    let adjust f k m =
        adjustWithKey (fun _ x -> f x) k m
    ...
    let append m1 m2 =
        mergeWithKey' (fun x y m1' m2' -> Bin(x, y, m1', m2')) konst id id m1 m2

    let appendWithKey f m1 m2 =
        mergeWithKey' (fun x y m1' m2' -> Bin(x, y, m1', m2')) (fun (Tip(k1, x1)) (Tip(_, x2)) -> Tip(k1, f k1 x1 x2)) id id m1 m2

    let appendWith f m1 m2 =
        appendWithKey (fun _ x y -> f x y) m1 m2

    let concat xs =
        List.fold append empty xs

    let concatWith f xs =
        List.fold (appendWith f) empty xs

    ///O(n+m). Difference between two maps (based on keys). Credit: Haskell.org
    let difference m1 m2 =
        mergeWithKey (fun _ _ _ -> None) id (konst Nil) m1 m2
    ...
    let differenceWithKey f m1 m2 =
        mergeWithKey f id (konst Nil) m1 m2

    ///O(n+m). Difference with a combining function. Credit: Haskell.org
    let differenceWith f m1 m2 =
        differenceWithKey (fun _ x y -> f x y) m1 m2

    ///O(n+m). The (left-biased) intersection of two maps (based on keys). Credit: Haskell.org
    let intersection m1 m2 =
        mergeWithKey' bin konst (konst Nil) (konst Nil) m1 m2
    ...
    ///O(n+m). The intersection with a combining function. Credit: Haskell.org
    let intersectionWith f m1 m2 =
        intersectionWithKey (fun _ x y -> f x y) m1 m2
    ...
    ///O(log n). Update the value at the maximal key. Credit: Haskell.org
    let updateMax f =
        updateMaxWithKey(konst f)

    ///O(log n). Update the value at the minimal key. Credit: Haskell.org
    let updateMin f =
        updateMinWithKey(konst f)
    ...
    let maxView t =
        Option.map (first snd) (maxViewWithKey t)
    ...
    let minView t =
        Option.map (first snd) (minViewWithKey t)
    ...
    let mapAccum f =
        mapAccumWithKey(fun a' _ x -> f a' x)
    ...
    ///O(n). partition the map according to some predicate. The first map contains all elements that satisfy the predicate, the second all elements that fail the predicate. See also split. Credit: Haskell.org
    let partition p m =
        partitionWithKey (fun _ x -> p x) m
    ...
    let split k t =
        ...
        | Bin(_, m, l, r) when m < 0 ->
            if
                k >= 0 // handle negative numbers.
            then
                let (lt, gt) = go k l in
                let lt = append r lt in
                (lt, gt)
            else
                let (lt, gt) = go k r in
                let gt = append gt l in
                (lt, gt)
        | _ -> go k t
    ...
    let splitTryFind k t =
        ...
        | Bin(_, m, l, r) when m < 0 ->
            if
                k >= 0 // handle negative numbers.
            then
                let (lt, fnd, gt) = go k l in
                let lt = append r lt in
                (lt, fnd, gt)
            else
                let (lt, fnd, gt) = go k r in
                let gt = append gt l in
                (lt, fnd, gt)
        | _ -> go k t
    ...
    let inline foldBackWithKey f z =
        fun (t: _ IntMap) -> t.FoldBackWithKey f z
    ...
    let ofSeqWithKey f xs =
        xs |> List.ofSeq |> ofListWithKey f
    ...
    let ofArrayWithKey f xs =
        xs |> List.ofArray |> ofListWithKey f
    ...
    let ofArrayWith f xs =
        xs |> List.ofArray |> ofListWith f
    ...
    let mapKeys f =
        ofList << foldBackWithKey (fun k x xs -> (f k, x) :: xs) []
    ...
    let mapKeysWith c f =
        ofListWith c << foldBackWithKey (fun k x xs -> (f k, x) :: xs) []
