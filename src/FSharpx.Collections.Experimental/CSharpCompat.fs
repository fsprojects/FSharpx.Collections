...
type RoseTree =
    ...
    static member Create(root, [<ParamArray>] children) =
        RoseTree.create root (LazyList.ofArray children)

    static member Create(root, children) =
        RoseTree.create root children

    [<Extension>]
    static member Select(tree, f: Func<_, _>) =
        RoseTree.map f.Invoke tree

    [<Extension>]
    static member SelectMany(o, f: Func<_, _>) =
        RoseTree.bind f.Invoke o
