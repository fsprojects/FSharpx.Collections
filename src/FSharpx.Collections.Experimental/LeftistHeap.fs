...
type LeftistHeap<'T when 'T: comparison> =
    ...
    static member private rank: LeftistHeap<'T> -> int =
        function
        | E(_) -> 0
        | T(_, _, r, _, _, _) -> r
        ...
        if (h1.IsDescending) = (h2.IsDescending) then
            ...
            | T(_, _, _, x, a1, b1), T(_, _, _, y, a2, b2) ->
                if (h1.IsDescending) then
                    if x < y then
                        LeftistHeap.make y a2 (LeftistHeap.merge h1 b2)
                    else
                        LeftistHeap.make x a1 (LeftistHeap.merge b1 h2)
                else if x < y then
                    LeftistHeap.make x a1 (LeftistHeap.merge b1 h2)
                else
                    LeftistHeap.make y a2 (LeftistHeap.merge h1 b2)
    ...
    static member private foldHeap nodeF leafV (h: LeftistHeap<'T>) =
        ...
        let rec Loop (h: LeftistHeap<'T>) cont =
            match h with
            | T(_, _, _, a, l, r) -> Loop l (fun lacc -> Loop r (fun racc -> cont(nodeF a lacc racc)))
            | E(_) -> cont leafV

        Loop h (fun x -> x)

    static member private inOrder(h: LeftistHeap<'T>) =
        (LeftistHeap.foldHeap (fun x l r acc -> l(x :: (r acc))) (fun acc -> acc) h) []

    static member private isEmpty: LeftistHeap<'T> -> bool =
        function
        | E(_) -> true
        | _ -> false
    ...
    static member private tryMerge (h1: LeftistHeap<'T>) (h2: LeftistHeap<'T>) : LeftistHeap<'T> option =
        if (h1.IsDescending) = (h2.IsDescending) then
            ...
            | T(_, _, _, x, a1, b1), T(_, _, _, y, a2, b2) ->
                if x < y then
                    Some(LeftistHeap.make x a1 (LeftistHeap.merge b1 h2))
                else
                    Some(LeftistHeap.make y a2 (LeftistHeap.merge h1 b2))
        else
            None
    ...
    static member private head: LeftistHeap<'T> -> 'T =
        function
        | E(_) -> raise Exceptions.Empty
        | T(_, _, _, x, _, _) -> x

    static member private tryGetHead: LeftistHeap<'T> -> 'T option =
        function
        | E(_) -> None
    ...
    static member internal ofSeq (descending: bool) (s: seq<'T>) : LeftistHeap<'T> =
        if Seq.isEmpty s then
            ...
            let x, _ =
                Seq.fold
                    (fun (acc, isDescending) elem ->
                        (((T(isDescending, 1, 1, elem, E(isDescending), E(isDescending))))
                         :: acc),
                        isDescending)
                    ([], descending)
                    s
            ...
            let pairWiseMerge(l: list<LeftistHeap<'T>>) =
                let rec loop(acc: list<LeftistHeap<'T>>) : list<LeftistHeap<'T>> -> list<LeftistHeap<'T>> =
                    function
            ...
            let rec loop: list<LeftistHeap<'T>> -> LeftistHeap<'T> =
                function
                | h :: [] -> h
    ...
    static member private tail: LeftistHeap<'T> -> LeftistHeap<'T> =
        function
        | E(_) -> raise Exceptions.Empty
        | T(_, _, _, _, a, b) -> LeftistHeap.merge a b

    static member private tryGetTail: LeftistHeap<'T> -> LeftistHeap<'T> option =
        function
        | E(_) -> None
    ...
    interface IHeap<LeftistHeap<'T>, 'T> with
        ...
        member this.Head() =
            LeftistHeap.head this

        member this.TryGetHead() =
            LeftistHeap.tryGetHead this
        ...
        member this.Merge(xs: LeftistHeap<'T>) =
            LeftistHeap.merge this xs
        ...
        member this.GetEnumerator() =
            let e =
                ...
                then
                    LeftistHeap.inOrder this |> List.sort |> List.rev |> List.toSeq
                else
                    LeftistHeap.inOrder this |> List.sort |> List.toSeq
...
module LeftistHeap =
    ...
    let (|Cons|Nil|)(h: LeftistHeap<'T>) =
        match h.TryUncons() with
        | Some(a, b) -> Cons(a, b)
        | None -> Nil

    ///O(1). Returns a empty heap.
    let inline empty(descending: bool) =
        E(descending)
    ...
    ///O(1). Returns option first min or max element.
    let inline tryGetHead(xs: LeftistHeap<'T>) =
        xs.TryGetHead

    ///O(log n). Returns a new heap with the element inserted.
    let inline insert x (xs: LeftistHeap<'T>) =
        xs.Insert x

    ///O(1). Returns true if the heap has no elements.
    let inline isEmpty(xs: LeftistHeap<'T>) =
        xs.IsEmpty

    ///O(1). Returns true if the heap has max element at head.
    let inline isDescending(xs: LeftistHeap<'T>) =
        xs.IsDescending
    ...
    ///O(log n). Returns heap from merging two heaps, both must have same isDescending.
    let inline merge (xs: LeftistHeap<'T>) (ys: LeftistHeap<'T>) =
        xs.Merge ys

    ///O(log n). Returns heap option from merging two heaps.
    let inline tryMerge (xs: LeftistHeap<'T>) (ys: LeftistHeap<'T>) =
        xs.TryMerge ys
    ...
    ///O(log n). Returns option heap of the elements trailing the head.
    let inline tryGetTail(xs: LeftistHeap<'T>) =
        xs.TryGetTail()

    ///O(log n). Returns the head element and tail.
    let inline uncons(xs: LeftistHeap<'T>) =
        xs.Uncons()

    ///O(log n). Returns option head element and tail.
    let inline tryUncons(xs: LeftistHeap<'T>) =
        xs.TryUncons()
