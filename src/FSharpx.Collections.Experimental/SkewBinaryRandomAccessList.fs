...
type SkewBinaryRandomAccessList<'T>(randomAccessList) =
    ...
    static member private length: int * list<int * TreeSBRAL<'T>> -> int =
        function
        | len, [] -> len
        | len, (0, _) :: ts -> SkewBinaryRandomAccessList.length(len, ts)
        | len, (i, _) :: ts -> SkewBinaryRandomAccessList.length((len + i), ts)

    static member private cons(x: 'T) : list<int * TreeSBRAL<'T>> -> list<int * TreeSBRAL<'T>> =
        function
        | [] -> [ (1, Leaf x) ]
    ...
    static member private head: list<int * TreeSBRAL<'T>> -> 'T =
        function
    ...
    static member private tryGetHead: list<int * TreeSBRAL<'T>> -> 'T option =
        function
        | [] -> None
        | (1, Leaf x) :: _ -> Some(x)
    ...
    static member private lookupTree: int * int * TreeSBRAL<'T> -> 'T =
        function
        | 1, 0, Leaf x -> x
    ...
    static member private tryLookupTree: int * int * TreeSBRAL<'T> -> 'T option =
        function
        | 1, 0, Leaf x -> Some(x)
        | 1, i, Leaf x -> None
    ...
    static member private lookup: int * list<int * TreeSBRAL<'T>> -> 'T =
        function
        | i, [] -> raise Exceptions.OutOfBounds
        | i, (w, t) :: ts ->
            if i < w then
                SkewBinaryRandomAccessList.lookupTree(w, i, t)
            else
                SkewBinaryRandomAccessList.lookup((i - w), ts)

    static member private tryLookup: int * list<int * TreeSBRAL<'T>> -> 'T option =
        function
        | i, [] -> None
        | i, (w, t) :: ts ->
            if i < w then
                SkewBinaryRandomAccessList.tryLookupTree(w, i, t)
            else
                SkewBinaryRandomAccessList.tryLookup((i - w), ts)

    static member private tail: list<int * TreeSBRAL<'T>> -> list<int * TreeSBRAL<'T>> =
        function
        | [] -> raise Exceptions.Empty
        | (1, Leaf _) :: ts -> ts
    ...
    static member updateTree: int * int * 'T * TreeSBRAL<'T> -> TreeSBRAL<'T> =
        function
        | 1, 0, y, Leaf x -> Leaf y
    ...
    static member tryUpdateTree: int * int * 'T * TreeSBRAL<'T> -> TreeSBRAL<'T> option =
        function
    ...
    static member update i y : list<int * TreeSBRAL<'T>> -> list<int * TreeSBRAL<'T>> =
        function
    ...
    static member tryUpdate i y : list<int * TreeSBRAL<'T>> -> list<int * TreeSBRAL<'T>> option =
        function
        | [] -> None
        | (w, t) :: ts ->
            if i < w then
                Some(
                    (w, (SkewBinaryRandomAccessList.tryUpdateTree(w, i, y, t)).Value)
                    :: ts
                )
            else
    ...
    static member internal ofSeq(s: seq<'T>) : SkewBinaryRandomAccessList<'T> =
        if Seq.isEmpty s then
            ...
            let rec loop (acc: list<int * TreeSBRAL<'T>>) dec (a': array<'T>) =
                if dec < 0 then
                    SkewBinaryRandomAccessList(acc)
                else
                    loop (SkewBinaryRandomAccessList.cons a'.[dec] acc) (dec - 1) a'

            loop [] (a.Length - 1) a

    ///O(1), worst case. Returns a new random access list with the element added to the beginning.
    member this.Cons(x: 'T) =
        SkewBinaryRandomAccessList(SkewBinaryRandomAccessList.cons x randomAccessList)
    ...
    member this.Length() =
        SkewBinaryRandomAccessList.length(0, randomAccessList)

    ///O(log n), worst case. Returns element by index.
    member this.Lookup(i: int) =
        SkewBinaryRandomAccessList.lookup(i, randomAccessList)

    ///O(log n), worst case. Returns option element by index.
    member thie.TryLookup(i: int) =
        SkewBinaryRandomAccessList.tryLookup(i, randomAccessList)

    ///O(n). Returns random access list reversed.
    member this.Rev() =

        let rec loop: list<int * TreeSBRAL<'T>> * list<int * TreeSBRAL<'T>> -> SkewBinaryRandomAccessList<'T> =
            function
    ...
    member this.Tail =
        SkewBinaryRandomAccessList(SkewBinaryRandomAccessList.tail randomAccessList)
    ...
    member this.Uncons = this.Head, this.Tail
    ...
    member this.Update i y =
        SkewBinaryRandomAccessList(SkewBinaryRandomAccessList.update i y randomAccessList)
...
module SkewBinaryRandomAccessList =
    ...
    let (|Cons|Nil|)(l: SkewBinaryRandomAccessList<'T>) =
    ...
    ///O(1), worst case. Returns option first element.
    let inline tryGetHead(xs: SkewBinaryRandomAccessList<'T>) =
        xs.TryGetHead
    ...
    ///O(1). Returns true if the random access list has no elements.
    let inline isEmpty(xs: SkewBinaryRandomAccessList<'T>) =
        xs.IsEmpty

    ///O(log n). Returns the count of elements.
    let inline length(xs: SkewBinaryRandomAccessList<'T>) =
        xs.Length()

    ///O(log n), worst case. Returns element by index.
    let inline lookup i (xs: SkewBinaryRandomAccessList<'T>) =
        xs.Lookup i

    ///O(log n), worst case. Returns option element by index.
    let inline tryLookup i (xs: SkewBinaryRandomAccessList<'T>) =
        xs.TryLookup i
    ...
    ///O(1), worst case. Returns a option random access list of the elements trailing the first element.
    let inline tryGetTail(xs: SkewBinaryRandomAccessList<'T>) =
        xs.TryGetTail
    ...
    ///O(1), worst case. Returns the option first element and tail.
    let inline tryUncons(xs: SkewBinaryRandomAccessList<'T>) =
        xs.TryUncons

    ///O(log n), worst case. Returns random access list with element updated by index.
    let inline update i y (xs: SkewBinaryRandomAccessList<'T>) =
        xs.Update i y

    ///O(log n), worst case. Returns option random access list with element updated by index.
    let inline tryUpdate i y (xs: SkewBinaryRandomAccessList<'T>) =
        xs.TryUpdate i y
