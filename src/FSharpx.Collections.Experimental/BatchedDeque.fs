...
type BatchedDeque<'T>(front, rBack) =
    ...
    member this.Init =
        ...
        | _, [] -> //splits front in two, favoring frontbot for odd length
            let _, fronttop, frontbot =
                List.fold
                    (fun (i, reartop, rearbot) e ->
                        if i < front.Length / 2 then
                            (i + 1, e :: reartop, rearbot)
                        else
                            (i + 1, reartop, e :: rearbot))
                    (0, [], [])
                    front
    ...
    member this.Rev = (new BatchedDeque<'T>(rBack, front))

    ///O(1). Returns a new deque with the element added to the end.
    member this.Snoc x =
        BatchedDeque(front, x :: rBack)
...
module BatchedDeque =
    ...
    let (|Cons|Nil|)(q: BatchedDeque<'T>) =
        match q.TryUncons with
        | Some(a, b) -> Cons(a, b)
        | None -> Nil

    let (|Snoc|Nil|)(q: BatchedDeque<'T>) =
    ...
    ///O(1) amortized, O(n), worst case. Returns option first element.
    let inline tryGetHead(q: BatchedDeque<'T>) =
        q.TryGetHead
    ...
    let inline tryGetInit(q: BatchedDeque<'T>) =
        q.TryGetInit
    ...
    ///O(1) amortized, O(n), worst case. Returns option last element.
    let inline tryGetLast(q: BatchedDeque<'T>) =
        q.TryGetLast
    ...
    ///O(n), worst case. Returns element by index.
    let inline lookup i (q: BatchedDeque<'T>) =
        q.Lookup i

    ///O(n), worst case. Returns option element by index.
    let inline tryLookup i (q: BatchedDeque<'T>) =
        q.TryLookup i
    ...
    ///O(n), worst case. Returns deque with element removed by index.
    let inline remove i (q: BatchedDeque<'T>) =
        q.Remove i

    ///O(n), worst case. Returns option deque with element removed by index.
    let inline tryRemove i (q: BatchedDeque<'T>) =
        q.TryRemove i
    ...
    ///O(1). Returns a new deque with the element added to the end.
    let inline snoc (x: 'T) (q: BatchedDeque<'T>) =
        (q.Snoc x)
    ...
    ///O(1) amortized, O(n), worst case. Returns option deque of the elements trailing the first element.
    let inline tryGetTail(q: BatchedDeque<'T>) =
        q.TryGetTail
    ...
    ///O(1) amortized, O(n), worst case. Returns option first element and tail.
    let inline tryUncons(q: BatchedDeque<'T>) =
        q.TryUncons
    ...
    ///O(1) amortized, O(n), worst case. Returns option init and the last element.
    let inline tryUnsnoc(q: BatchedDeque<'T>) =
        q.TryUnsnoc

    ///O(n), worst case. Returns deque with element updated by index.
    let inline update i y (q: BatchedDeque<'T>) =
        q.Update i y

    ///O(n), worst case. Returns option deque with element updated by index.
    let inline tryUpdate i y (q: BatchedDeque<'T>) =
        q.TryUpdate i y
