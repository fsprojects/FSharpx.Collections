...
module private SBHTree =
    ...
    let link descending (Node(x, auxX, childrenX) as treeX) (Node(y, auxY, childrenY) as treeY) =
        if x <= y <> descending then
            Node(x, auxX, treeY :: childrenX)
        else
            Node(y, auxY, treeX :: childrenY)

    let skewLink descending v treeX treeY =
        let (Node(w, aux, children)) = link descending treeX treeY

        if v <= w <> descending then
            Node(v, w :: aux, children)
        else
            Node(w, v :: aux, children)
...
module private SBHTreeRoot =
    ...
    let rec extractMinRoot descending =
        ...
        | (Root(_, tree) as root) :: roots ->
            ...
            if (SBHTree.item tree <= SBHTree.item tree') <> descending then
                root, roots
            else
                root', root :: roots'
        | [] -> bug "[]"
    ...
    let uncons descending roots =
        ...
        let (_, reversed) =
            children
            |> List.fold (fun (rank, trees) tree -> rank - 1, Root(rank - 1, tree) :: trees) (rank, [])
        ...
        let newRoots =
            aux
            |> List.fold (fun roots value -> insert descending value roots) merged
    ...
    let rec toListOrdered descending roots =
        ...
        let sorted = [ (roots |> List.map getTree) ] |> treeToList [] |> List.sort
...
type 'T SkewBinomialHeap when 'T: comparison private (count, descending, roots: 'T SBHTreeRoot list) =
        ...
        if descending = other.IsDescending then
            Some(
                SkewBinomialHeap(
                    count + other.Count,
                    descending,
                    SBHTreeRoot.mergeRoots descending (SBHTreeRoot.normalize descending roots) (SBHTreeRoot.normalize descending other.Roots)
                )
            )
        else
            None
    ...
    member __.TryHead() =
        if count = 0 then
            ...
            Some(SBHTreeRoot.findMinRootItem descending roots)
    ...
    member this.TryUncons() =
        if this.IsEmpty then
            ...
            let (h, t) = SBHTreeRoot.uncons descending roots in Some(h, SkewBinomialHeap(count - 1, descending, t))
    ...
    interface IReadOnlyCollection<'T> with
        ...
        member __.GetEnumerator() =
            (SBHTreeRoot.toListOrdered descending roots |> List.toSeq).GetEnumerator()

        member this.GetEnumerator() : System.Collections.IEnumerator =
            upcast (this :> _ seq).GetEnumerator()
    ...
    interface IPriorityQueue<'T> with
        ...
        member this.TryPop() =
            this.TryUncons() |> Option.map(fun (h, t) -> h, upcast t)

[<RequireQualifiedAccess>]
module SkewBinomialHeap =

    let inline (|Cons|Nil|)(heap: 'T SkewBinomialHeap) =
        match heap.TryUncons() with
        | Some(h, t) -> Cons(h, t)
        | None -> Nil

    ///O(1) - Returns an empty heap.
    let inline empty descending =
    ...
    let inline tryHead(xs: 'T SkewBinomialHeap) =
        xs.TryHead()

    ///O(1) - Returns a new heap with the element inserted.
    let inline insert x (xs: 'T SkewBinomialHeap) =
        xs.Insert x

    ///O(1) - Returns true if the heap has no elements.
    let inline isEmpty(xs: 'T SkewBinomialHeap) =
        xs.IsEmpty
    ...
    let inline isDescending(xs: 'T SkewBinomialHeap) =
        xs.IsDescending
    ...
    let inline merge (xs: 'T SkewBinomialHeap) (ys: 'T SkewBinomialHeap) =
        xs.Merge ys
    ...
    let inline tryMerge (xs: 'T SkewBinomialHeap) (ys: 'T SkewBinomialHeap) =
        xs.TryMerge ys

    ///O(n) - Returns heap from the sequence.
    let ofSeq descending s =
        s |> Seq.fold (fun heap value -> insert value heap) (empty descending)
