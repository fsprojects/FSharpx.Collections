...
type HoodMelvilleQueue<'T>(frontLength: int, front: list<'T>, state: RotationState<'T>, rBackLength: int, rBack: list<'T>) =
    ...
    static member private exec: RotationState<'T> -> RotationState<'T> =
        function
    ...
    static member private invalidate: RotationState<'T> -> RotationState<'T> =
        function
    ...
    static member private check(q: HoodMelvilleQueue<'T>) =
        if q.rBackLength <= q.frontLength then
            ...
            HoodMelvilleQueue((q.frontLength + q.rBackLength), q.front, newstate, 0, [])
            |> HoodMelvilleQueue.exec2
    ...
    static member internal Empty() : HoodMelvilleQueue<'T> =
        HoodMelvilleQueue(0, [], Idle, 0, [])
    ...
    static member internal OfList(xs: list<'T>) =
        HoodMelvilleQueue<'T>(xs.Length, xs, Idle, 0, [])
...
module HoodMelvilleQueue =
    ...
    let (|Cons|Nil|)(q: HoodMelvilleQueue<'T>) =
    ...
    let fold (f: ('State -> 'T -> 'State)) (state: 'State) (q: HoodMelvilleQueue<'T>) =
        HoodMelvilleQueue<_>.fold f state q

    ///applies a function to each element of the queue, threading an accumulator argument through the computation, right to left
    let foldBack (f: ('T -> 'State -> 'State)) (q: HoodMelvilleQueue<'T>) (state: 'State) =
        HoodMelvilleQueue<_>.foldBack f q state

    ///returns the first element
    let inline head(q: HoodMelvilleQueue<'T>) = q.Head

    ///returns option first element
    let inline tryGetHead(q: HoodMelvilleQueue<'T>) =
        q.TryGetHead
    ...
    ///returns a new queue with the element added to the end
    let inline snoc (x: 'T) (q: HoodMelvilleQueue<'T>) =
        (q.Snoc x)
    ...
    ///returns option queue of the elements trailing the first element
    let inline tryGetTail(q: HoodMelvilleQueue<'T>) =
        q.TryGetTail
    ...
    ///returns option first element and tail
    let inline tryUncons(q: HoodMelvilleQueue<'T>) =
        q.TryUncons
