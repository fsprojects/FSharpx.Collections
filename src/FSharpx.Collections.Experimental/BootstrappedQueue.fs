...
type NonEmptyBootstrappedQueue<'T> =
    ...
    static member create lenfm f m lenr r = { FrontAndSuspensionsLength = lenfm
                                              Front = f
                                              Suspensions = m
                                              RBackLength = lenr
                                              RBack = r }
...
and BootstrappedQueue<'T> =
    ...
    static member checkF(q: NonEmptyBootstrappedQueue<'T>) : BootstrappedQueue<'T> =
        ...
        | [], m ->
            ...
            NonEmpty
            <| NonEmptyBootstrappedQueue<'T>.create q.FrontAndSuspensionsLength f susp q.RBackLength q.RBack
        | _ -> NonEmpty q

    static member snoc(x: 'T) : BootstrappedQueue<'T> -> BootstrappedQueue<'T> =
        function
    ...
    static member head: BootstrappedQueue<'T> -> 'T =
        function
        | Empty -> raise Exceptions.Empty
        | NonEmpty q -> List.head q.Front

    static member tryGetHead: BootstrappedQueue<'T> -> 'T option =
        function
        | Empty -> None
        | NonEmpty q -> Some(List.head q.Front)

    static member tail: BootstrappedQueue<'T> -> BootstrappedQueue<'T> =
        function
        | Empty -> raise Exceptions.Empty
        | NonEmpty q ->
    ...
    static member tryGetTail: BootstrappedQueue<'T> -> BootstrappedQueue<'T> option =
        function
        | Empty -> None
        | NonEmpty q ->
            let lenfm = q.FrontAndSuspensionsLength - 1
    ...
    static member length: BootstrappedQueue<'T> -> int =
        function
        | Empty -> 0
