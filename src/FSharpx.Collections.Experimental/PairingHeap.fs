...
type PairingHeap<'T when 'T: comparison> =
    ...
    static member private descending: PairingHeap<'T> -> bool =
        function
        | E(m) -> m
        | T(m, _, _) -> m
    ...
    static member private foldHeap nodeF leafV (h: list<PairingHeap<'T>>) =
        ...
        let rec loop (h: list<PairingHeap<'T>>) cont =
            match h with
            | T(_, a, h') :: tl -> loop h' (fun lacc -> loop tl (fun racc -> cont(nodeF a lacc racc)))
            | _ -> cont leafV

        loop h (fun x -> x)

    static member private inOrder(h: list<PairingHeap<'T>>) =
        (PairingHeap.foldHeap (fun x l r acc -> l(x :: (r acc))) (fun acc -> acc) h) []

    static member private sumTree(h: list<PairingHeap<'T>>) =
        (PairingHeap.foldHeap (fun x l r acc -> l(r(acc + 1))) (fun acc -> acc) h) 0

    static member private isEmpty: PairingHeap<'T> -> bool =
        function
        | E(_) -> true
        | _ -> false

    static member private tryMerge (h1: PairingHeap<'T>) (h2: PairingHeap<'T>) : PairingHeap<'T> option =
        if (PairingHeap.descending h1) = (PairingHeap.descending h2) then
            Some(PairingHeap.merge h1 h2)
        else
            None
    ...
    static member private head: PairingHeap<'T> -> 'T =
        function
        | E(_) -> raise Exceptions.Empty
        | T(_, x, _) -> x

    static member private tryGetHead: PairingHeap<'T> -> 'T option =
        function
        | E(_) -> None
        | T(_, x, _) -> Some(x)
    ...
    static member internal ofSeq (isDescending: bool) (s: seq<'T>) : PairingHeap<'T> =
        ...
        //and has the further advantage of being only one line of code
            ...
            Seq.fold (fun (h: 'T PairingHeap) t -> (h.Insert t)) (E(isDescending)) s

    static member private tail: PairingHeap<'T> -> PairingHeap<'T> =
        function
        | E(_) -> raise Exceptions.Empty
        | T(m, x, xs) ->
    ...
    static member private tryGetTail: PairingHeap<'T> -> PairingHeap<'T> option =
        function
        | E(_) -> None
        | T(m, x, xs) ->
            let rec mergePairs pairList isDescending =
    ...
    interface IReadOnlyCollection<'T> with
        ...
        member this.GetEnumerator() =
            let e =
                ...
                then
                    PairingHeap.inOrder listH |> List.sort |> List.rev |> List.toSeq
                else
                    PairingHeap.inOrder listH |> List.sort |> List.toSeq
...
module PairingHeap =
    ...
    let (|Cons|Nil|)(h: PairingHeap<'T>) =
        match h.TryUncons() with
        | Some(a, b) -> Cons(a, b)
        | None -> Nil

    ///O(1) Returns a empty heap.
    let inline empty(maximalist: bool) =
        E(maximalist)
    ...
    ///O(1) worst case. Returns option first min or max element.
    let inline tryGetHead(xs: PairingHeap<'T>) =
        xs.TryGetHead()

    ///O(1) amortized time. Returns a new heap with the element inserted.
    let inline insert x (xs: PairingHeap<'T>) =
        xs.Insert x

    ///O(1) Returns true if the heap has no elements.
    let inline isEmpty(xs: PairingHeap<'T>) =
        xs.IsEmpty

    ///O(1). Returns true if the heap has max element at head.
    let inline isDescending(xs: PairingHeap<'T>) =
        xs.IsDescending

    ///O(n). Returns the count of elements.
    let inline length(xs: PairingHeap<'T>) =
        xs.Length()

    ///O(1) amortized time. Returns heap from merging two heaps, both must have same descending.
    let inline merge (xs: PairingHeap<'T>) (ys: PairingHeap<'T>) =
        xs.Merge ys

    ///O(1) amortized time. Returns heap option from merging two heaps.
    let inline tryMerge (xs: PairingHeap<'T>) (ys: PairingHeap<'T>) =
        xs.TryMerge ys

    ///O(n). Returns heap from the sequence.
    let ofSeq maximalist s =
    ...
    ///O(log n) amortized time. Returns option heap of the elements trailing the head.
    let inline tryGetTail(xs: PairingHeap<'T>) =
        xs.TryGetTail()

    ///O(log n) amortized time. Returns the head element and tail.
    let inline uncons(xs: PairingHeap<'T>) =
        xs.Uncons()

    ///O(log n) amortized time. Returns option head element and tail.
    let inline tryUncons(xs: PairingHeap<'T>) =
        xs.TryUncons()
