...
type DList<'T>(length: int, data: DListData<'T>) =
        ...
        DList(
            ...
            (Seq.fold
                (fun state x ->
                    ...
                    | Join(_, _) as xs -> Join(state, Unit x))
                Nil
                s)
        )
    ...
    member this.TryTail =
        ...
        if this.IsEmpty then
            ...
            Some(DList((length - 1), (step data Nil)))
...
module DList =
    ...
    let inline tryUncons(l: DList<'T>) =
        l.TryUncons
    ...
    let inline toList l =
        foldBack (List.cons) l []

    let inline toSeq(l: DList<'T>) =
        l :> seq<'T>
    ...
    let rec pairWiseDListData cons dlist lastvalue =
        ...
        | Cons(x, rest) -> pairWiseDListData (Join(cons, Unit(lastvalue, x))) rest x
    ...
    let pairwise(l: DList<'T>) =
        let dlistData =
            ...
            //| Unit _ -> Nil
            | Cons(x, Nil) -> Nil
            | Cons(x, (Cons(y, rest))) -> pairWiseDListData (Unit(x, y)) rest y
