...
module ResizeArray =
    ...
    let set (arr: ResizeArray<'T>) (n: int) (x: 'T) =
        arr.[n] <- x
    ...
    let concat(arrs: seq<ResizeArray<'T>>) =
        new ResizeArray<_>(
            seq {
                for arr in arrs do
                    for x in arr do
    ...
    let copy(arr: ResizeArray<'T>) =
        new ResizeArray<_>(arr)
    ...
    let toArray(arr: ResizeArray<'T>) =
        arr.ToArray()

    let ofArray(arr: 'T[]) =
        new ResizeArray<_>(arr)

    let toSeq(arr: ResizeArray<'T>) =
        Seq.readonly arr

    let sort f (arr: ResizeArray<'T>) =
