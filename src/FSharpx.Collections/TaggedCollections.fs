...
module SetTree =
    ...
    let rec remove (comparer: IComparer<'T>) k t =
        ...
        | SetNode(k2, l, r, _) ->
            ...
            if c < 0 then
                rebalance (remove comparer k l) k2 r
            elif c = 0 then
                match l, r with
                | SetEmpty, _ -> r
                | _, SetEmpty -> l
                ...
                rebalance l k2 (remove comparer k r)
    ...
    let rec union comparer t1 t2 =
        ...
        | SetNode(k1, t11, t12, h1), SetNode(k2, t21, t22, h2) -> // (t11 < k < t12) AND (t21 < k2 < t22)
            ...
            if h1 > h2 then
                let lo, _, hi = split comparer k1 t2 in balance comparer (union comparer t11 lo) k1 (union comparer t12 hi)
            else
                let lo, _, hi = split comparer k2 t1 in balance comparer (union comparer t21 lo) k2 (union comparer t22 hi)
        | SetEmpty, t -> t
        | t, SetEmpty -> t
#if ONE
...
#if ONE
        | SetOne(k) -> if contains comparer k b then add comparer k acc else acc
#endif
        | SetEmpty -> acc
    ...
    let rec nextElemCont (comparer: IComparer<'T>) k s cont =
        ...
        | MatchSetNode(k2, l, r) ->
            ...
            if c < 0 then
                ...
                cont(minimumElementOpt r)
            else
                nextElemCont comparer k r cont
        | MatchSetEmpty -> cont(None)

    and nextElem comparer k s =
        nextElemCont comparer k s (fun res -> res)
    ...
    and prevElemCont (comparer: IComparer<'T>) k s cont =
        ...
        | MatchSetNode(k2, l, r) ->
            ...
            if c > 0 then
                ...
                cont(maximumElementOpt r)
            else
                prevElemCont comparer k l cont
        | MatchSetEmpty -> cont(None)

    and prevElem comparer k s =
        prevElemCont comparer k s (fun res -> res)
    ...
    let rec compareStacks (comparer: IComparer<'T>) l1 l2 =
        ...
        | (SetOne(n1k) :: t1), (SetNode(n2k, SetEmpty, n2r, _) :: t2) ->
            ...
            if c <> 0 then
                ...
                compareStacks comparer (empty :: t1) (n2r :: t2)
        | (SetNode(n1k, (SetEmpty as emp), n1r, _) :: t1), (SetOne(n2k) :: t2) ->
            ...
            if c <> 0 then
                ...
                compareStacks comparer (n1r :: t1) (emp :: t2)
#endif
        | (SetNode(n1k, SetEmpty, n1r, _) :: t1), (SetNode(n2k, SetEmpty, n2r, _) :: t2) ->
            ...
            if c <> 0 then
                ...
                compareStacks comparer (n1r :: t1) (n2r :: t2)
#if ONE
        | (SetOne(n1k) :: t1), _ -> compareStacks comparer (empty :: SetOne(n1k) :: t1) l2
#endif
        | (SetNode(n1k, n1l, n1r, _) :: t1), _ -> compareStacks comparer (n1l :: SetNode(n1k, empty, n1r, 0) :: t1) l2
#if ONE
        | _, (SetOne(n2k) :: t2) -> compareStacks comparer l1 (empty :: SetOne(n2k) :: t2)
#endif
        | _, (SetNode(n2k, n2l, n2r, _) :: t2) -> compareStacks comparer l1 (n2l :: SetNode(n2k, empty, n2r, 0) :: t2)
...
type Set<'T, 'ComparerTag> when 'ComparerTag :> IComparer<'T>(comparer: IComparer<'T>, tree: SetTree<'T>) =
    ...
    static let refresh (s: Set<_, _>) t =
        Set<_, _>(comparer = s.ComparerUntyped, tree = t)
    ...
    member s.Contains(x) =
        SetTree.contains comparer x tree

    member s.Iterate(x) =
        SetTree.iter x tree

    member s.Fold f x =
        SetTree.fold f tree x
    ...
    interface System.IComparable with
        ...
        member s1.CompareTo(s2: obj) =
            SetTree.compare s1.ComparerUntyped s1.Tree ((s2 :?> Set<'T, 'ComparerTag>).Tree)
    ...
    interface ICollection<'T> with
        ...
        member s.CopyTo(arr, i) =
            SetTree.copyToArray tree arr i
...
module MapTree =
        ...
        if height t2 > t1h + 2 then (* right is heavier than left *)
            ...
            | MapNode(t2k, t2v, t2l, t2r, _) ->
                ...
                if height t2l > t1h + 1 then (* balance left: combination *)
                    match t2l with
                    | MapNode(t2lk, t2lv, t2ll, t2lr, _) -> mk (mk t1 k v t2ll) t2lk t2lv (mk t2lr t2k t2v t2r)
                    | _ -> failwith "rebalance"
                else (* rotate left *)
            ...
            if t1h > t2h + 2 then (* left is heavier than right *)
                ...
                | MapNode(t1k, t1v, t1l, t1r, _) ->
                    ...
                    if height t1r > t2h + 1 then
                        (* balance right: combination *)
                        match t1r with
                        | MapNode(t1rk, t1rv, t1rl, t1rr, _) -> mk (mk t1l t1k t1v t1rl) t1rk t1rv (mk t1rr k v t2)
                        | _ -> failwith "rebalance"
                    else
    ...
    let rec find (comparer: IComparer<'T>) k m =
        ...
        | MapOne(k2, v2) ->
            let c = comparer.Compare(k, k2)
            if c = 0 then v2 else indexNotFound()
#endif
        | MapNode(k2, v2, l, r, _) ->
            let c = comparer.Compare(k, k2)
    ...
    let filter (comparer: IComparer<'T>) f s =
        filterAux comparer f s empty
    ...
    let rec remove (comparer: IComparer<'T>) k m =
        ...
        | MapNode(k2, v2, l, r, _) ->
            ...
            if c < 0 then
                rebalance (remove comparer k l) k2 v2 r
            elif c = 0 then
                match l, r with
                | MapEmpty, _ -> r
                | _, MapEmpty -> l
    ...
    let rec containsKey (comparer: IComparer<'T>) k m =
        ...
        | MapNode(k2, _, l, r, _) ->
            ...
            if c < 0 then
                containsKey comparer k l
            else
                (c = 0 || containsKey comparer k r)
...
type Map<'Key, 'T, 'ComparerTag> when 'ComparerTag :> IComparer<'Key>(comparer: IComparer<'Key>, tree: MapTree<'Key, 'T>) =
    ...
    static let refresh (m: Map<_, _, 'ComparerTag>) t =
        Map<_, _, 'ComparerTag>(comparer = m.ComparerUntyped, tree = t)
    ...
    member m.First(f) =
        MapTree.first f tree

    member m.Exists(f) =
        MapTree.exists f tree

    member m.Filter(f) =
        MapTree.filter comparer f tree |> refresh m

    member m.ForAll(f) =
        MapTree.forAll f tree

    member m.Fold f acc =
        MapTree.fold f tree acc
    ...
    member m.MapRange f =
        refresh m (MapTree.map f tree)

    member m.Map f =
        refresh m (MapTree.mapi f tree)
    ...
    member m.ContainsKey(k) =
        MapTree.containsKey comparer k tree
    ...
    interface Collections.Generic.IEnumerable<KeyValuePair<'Key, 'T>> with
        member s.GetEnumerator() =
            MapTree.toSeq tree
