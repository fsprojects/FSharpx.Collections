...
module ByteString =
    ...
    /// needs .fsi file
    let singleton c =
        ByteString(Array.create 1 c, 0, 1)
...
#if !FABLE_COMPILER
    /// needs .fsi file
    let ofArraySegment(segment: ArraySegment<byte>) =
        ByteString(segment.Array, segment.Offset, segment.Count)
#endif

    /// needs .fsi file
    let ofSeq s =
        let arr = Array.ofSeq s in ByteString(arr, 0, arr.Length)

    /// needs .fsi file
    let ofList l =
        ByteString(Array.ofList l, 0, l.Length)

    /// needs .fsi file
    let ofString(s: string) =
        s.ToCharArray() |> Array.map byte |> create
    ...
    let toArray(bs: ByteString) =
        if bs.Count = 0 then
            ...
            bs.Array.[bs.Offset .. (bs.Offset + bs.Count - 1)]

    /// needs .fsi file
    let toSeq(bs: ByteString) =
        bs :> seq<byte>

    /// needs .fsi file
    let toList(bs: ByteString) =
        List.ofSeq bs

    /// needs .fsi file
#if FABLE_COMPILER
    let toString(bs: ByteString) =
        System.Text.Encoding.UTF8.GetString(bs.Array, bs.Offset, bs.Count)
#else
    let toString(bs: ByteString) =
    ...
    /// needs .fsi file
    let length(bs: ByteString) = bs.Count

    /// needs .fsi file
    let index (bs: ByteString) pos = bs.[pos]
    ...
    let tail(bs: ByteString) =
        if bs.Count = 1 then
            ...
            ByteString(bs.Array, bs.Offset + 1, bs.Count - 1)
    ...
    let split pred (bs: ByteString) =
        if isEmpty bs then
            ...
            if index = -1 then
                ...
                ByteString(bs.Array, bs.Offset, count), ByteString(bs.Array, index, bs.Count - count)
#endif
    ...
    let splitAt n (bs: ByteString) =
        if isEmpty bs then
            ...
            empty, bs
        elif n >= bs.Count then
            bs, empty
        else
            let x, o, l = bs.Array, bs.Offset, bs.Count in ByteString(x, o, n), ByteString(x, o + n, l - n)
...
#if !FABLE_COMPILER
    /// needs .fsi file
    let skipWhile pred bs =
        span pred bs |> snd
#endif

#if !FABLE_COMPILER
    /// needs .fsi file
    let skipUntil pred bs =
        split pred bs |> snd
#endif
...
#if !FABLE_COMPILER
    /// needs .fsi file
    let takeWhile pred bs =
        span pred bs |> fst
#endif

#if !FABLE_COMPILER
    /// needs .fsi file
    let takeUntil pred bs =
        split pred bs |> fst
#endif
