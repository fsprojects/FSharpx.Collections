...
module Seq =
    ...
    let unCons s =
        if Seq.isEmpty s then None else Some(Seq.head s, Seq.tail s)
    ...
    let ofStreamByChunk chunkSize (stream: System.IO.Stream) =
        ...
        seq {
            while stream.Length <> stream.Position do
                ...
                if bytesRead = 0 then () else yield buffer
        }
#endif
    ...
    let grow n =
        Seq.collect(fun x -> (repeat x) |> Seq.take n)
...
module Array =
    ...
    let centeredWindow n (source: _[]) =
        if n < 0 then
            invalidArg "n" "n must be a positive integer"
    ...
    let inline centralMovingAverageOfOption n (a: ^t option array) =
        ...
        |> Array.map(fun window ->
            if Array.exists Option.isNone window then
                None
            else
                Some(Array.averageBy Option.get window))

    /// The catOptions function takes a list of Options and returns an array of all the Some values.
    let inline catOptions(xs: Option<'a>[]) =
        Array.choose id xs
...
module List =
    ...
    let skipWhile pred l =
        span pred l |> snd

    let skipUntil pred l =
        split pred l |> snd

    let takeWhile pred l =
        span pred l |> fst

    let takeUntil pred l =
        split pred l |> fst
    ...
    let pad (amt: int) (elem: 'a) (list: 'a list) : 'a list =
        if amt >= 0 then list @ (List.replicate amt elem) else list
    ...
    let inline catOptions(xs: Option<'a> list) =
        List.choose id xs
    ...
    let inline partitionChoices(xs: Choice<'a, 'b> list) =
        let inline choice f1 f2 a =
            match a with
            | Choice1Of2 x -> f1 x
            | Choice2Of2 y -> f2 y

        let inline choice1 x (c1, c2) =
            (cons x c1, c2)

        let inline choice2 y (c1, c2) =
            (c1, cons y c2)
    ...
    let inline equalsWith eq xs ys =
        List.compareWith (fun x y -> if eq x y then 0 else 1) xs ys = 0
...
module Map =
    ...
    let splitWithKey pred d =
        spanWithKey (not << pred) d
    ...
    let choose (f: 'T -> 'b -> 'c option) (map: Map<'T, 'b>) =
        ...
        ||> Map.fold(fun s k v ->
            let result = f k v
            if Option.isSome result then Map.add k result.Value s else s)
...
module NameValueCollection =
    ...
    /// <param name="a"></param>
    /// <param name="b"></param>
    let inline addInPlace (x: NameValueCollection) (a, b) =
        x.Add(a, b)
        ...
        { new IDictionary<string, string[]> with
            ...
            member d.Item
                with get k =
                    ...
                    if v = null then
                        raise <| KeyNotFoundException(sprintf "Key '%s' not found" k)
                    else
                        v
                and set k v =
                    x.Remove k
            ...
            member d.Add(key, value) =
                ...
                if d.ContainsKey key then
                    raise <| ArgumentException(sprintf "Duplicate key '%s'" key, "key")

                d.[key] <- value

            member d.Clear() = x.Clear()

            member d.Contains item =
                x.GetValues(item.Key) = item.Value

            member d.ContainsKey key =
                x.[key] <> null
    ...
    let asReadonlyDictionary x =
        ...
        let notSupported() =
            raise <| NotSupportedException("Readonly dictionary")
        ...
        { new IDictionary<string, string[]> with
            ...
            member d.CopyTo(array, arrayIndex) =
