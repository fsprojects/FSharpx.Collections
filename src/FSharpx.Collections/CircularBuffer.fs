...
type CircularBuffer<'T>(bufferSize: int) =
    do
        if bufferSize <= 0 then
            invalidArg "bufferSize" "The bufferSize must be greater than 0."
    ...
    member this.Dequeue(count) =
        ...
        if count > bufferSize then
            raise
            <| new ArgumentOutOfRangeException("Requested count exceeds the buffer size.")

        let count = min count length

        let dequeued =
            Array.concat [| for o, c in nextBuffer tail count -> buffer.[o .. o + c - 1] |]
