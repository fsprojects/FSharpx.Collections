module FSharpx.Collections.Experimental.Tests.Properties
...
let checkEquality<'a when 'a: equality> name =
    ...
    fsCheck(n "identity") <| fun (x: 'a) -> x = x
    ...
    fun (x: 'a) (y: 'a) (z: 'a) -> if x = y && y = z then x = z else true
...
*)
    let listBool n =
        Gen.listOfLength n Arb.generate<bool>

    let listInt n =
        Gen.listOfLength n Arb.generate<int>

    let listObj n =
        Gen.listOfLength n Arb.generate<obj>

    let listString n =
        Gen.listOfLength n Arb.generate<string>
