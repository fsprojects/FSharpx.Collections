...
module ImplicitQueueTest =
    ...
    let testImplicitQueue =
        ...
        testList
            "Experimental ImplicitQueue"
            [ test "ImplicitQueue.empty queue should be ImplicitQueue.empty" { ImplicitQueue.isEmpty ImplicitQueue.empty |> Expect.isTrue "" }

              test "it should allow to enqueue" {
                  ImplicitQueue.empty
                  |> ImplicitQueue.snoc 1
                  |> ImplicitQueue.snoc 2
              ...
              test "it should allow to get the ImplicitQueue.tail from the queue" {
                  ImplicitQueue.empty
                  |> ImplicitQueue.snoc "a"
                  |> ImplicitQueue.snoc "b"
                  |> ImplicitQueue.snoc "c"
                  |> ImplicitQueue.tail
                  |> ImplicitQueue.head
                  |> Expect.equal "" "b"
              }
