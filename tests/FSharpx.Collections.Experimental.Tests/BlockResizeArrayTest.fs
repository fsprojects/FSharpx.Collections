    ...
    let testBlockResizeArray =
        ...
        testList "Experimental BlockResizeArray" [
                                                   ...
                                                   test "random access performance" {
                                                       ...
                                                       averageTime testIters "ResizeArray random access" (fun () ->
                                                           for i in access do
                                                               ra.[i] <- 0UL)

                                                       averageTime testIters "Array random access" (fun () ->
                                                           for i in access do
                                                               a.[i] <- 0UL)

                                                       averageTime testIters "BlockResizeArray random access" (fun () ->
                                                           for i in access do
                                                               bra.[i] <- 0UL)
                                                   }
                                                   ...
                                                   test "sequential access performance" {
                                                       ...
                                                       averageTime testIters "ResizeArray sequential access" (fun () ->
                                                           for i in access do
                                                               ra.[i] <- 0UL)

                                                       averageTime testIters "Array sequential access" (fun () ->
                                                           for i in access do
                                                               a.[i] <- 0UL)

                                                       averageTime testIters "BlockResize sequential access" (fun () ->
                                                           for i in access do
                                                               bra.[i] <- 0UL)
                                                   }
                                                   ...
                                                   test "fold performance" {
                                                       ...
                                                       averageTime testIters "ResizeArray fold" (fun () ->
                                                           (ResizeArray.fold (fun x acc -> acc + x * 2UL) 0UL ra))

                                                       averageTime testIters "Array fold" (fun () -> (Array.fold (fun x acc -> acc + x * 2UL) 0UL a))

                                                       averageTime testIters "BlockResizeArray fold" (fun () ->
                                                           (FSharpx.Collections.Experimental.BlockResizeArray.fold
                                                               (fun x acc -> acc + x * 2UL)
                                                               0UL
                                                               bra))
                                                   }
                                                   ...
                                                   test "find performance" {
                                                       ...
                                                       averageTime testIters "ResizeArray find" (fun () ->
                                                           ResizeArray.find (fun e -> e <> 0 && e % s = 0) ra)

                                                       averageTime testIters "Array find" (fun () -> (Array.find (fun e -> e <> 0 && e % s = 0) a))

                                                       averageTime testIters "BlockResizeArray find" (fun () ->
                                                           (FSharpx.Collections.Experimental.BlockResizeArray.find (fun e -> e <> 0 && e % s = 0) bra))
                                                   }
#endif
