...
module BKTreeTest =
    ...
    let sem tree =
        tree |> BKTree.toList |> List.sort

    let trans f xs =
        xs |> BKTree.Int.ofList |> f |> sem
    ...
    let testBKTree =
        ...
        testList "Experimental BKTree" [
            ...
            <| fun (xs: int list) ->
                BKTree.List.distance [] xs = List.length xs
                && BKTree.List.distance xs [] = List.length xs

            testPropertyWithConfig config10k "native cons"
            <| fun (x: int) xs ys -> BKTree.List.distance (x :: xs) (x :: ys) = BKTree.List.distance xs ys

            testPropertyWithConfig config10k "native diff"
            <| fun (x: int) y xs ys ->
                x <> y
                ==> (BKTree.List.distance (x :: xs) (y :: ys) = 1
                                                                + List.min [
                                                                    BKTree.List.distance (x :: xs) ys
                                                                    BKTree.List.distance (x :: xs) (x :: ys)
                                                                    BKTree.List.distance xs (y :: ys)
                                                                ])
            ...
            <| fun dist n xs ->
                ...
                Prop.collect reference (BKTree.Int.existsDistance d n (BKTree.Int.ofList xs) = List.exists (fun e -> BKTree.Int.distance n e <= d) xs)
            ...
            <| fun dist n xs ->
                ...
                List.sort(BKTree.Int.toListDistance d n (BKTree.Int.ofList xs)) = List.sort(List.filter (fun e -> BKTree.Int.distance n e <= d) xs)
            ...
            <| fun xs ys -> sem(BKTree.Int.append (BKTree.Int.ofList xs) (BKTree.Int.ofList ys)) = List.sort(List.append xs ys)
