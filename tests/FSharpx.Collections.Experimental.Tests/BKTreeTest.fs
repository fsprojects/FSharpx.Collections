...
module BKTreeTest =
    ...
    let testBKTree =
        ...
        testList "Experimental BKTree" [ testPropertyWithConfig config10k "native empty"
                                         <| fun (xs: int list) ->
                                         ...
                                         <| fun (x: int) y xs ys ->
                                             ...
                                             ==> (BKTree.List.distance (x :: xs) (y :: ys) = 1
                                                                                             + List.min [ BKTree.List.distance (x :: xs) ys
                                         ...
                                         <| fun dist n xs ->
                                             ...
                                             Prop.collect
                                                 reference
                                                 (BKTree.Int.existsDistance d n (BKTree.Int.ofList xs) = List.exists
                                                                                                             (fun e -> BKTree.Int.distance n e <= d)
                                                                                                             xs)
                                         ...
                                         <| fun dist n xs ->
                                             ...
                                             List.sort(BKTree.Int.toListDistance d n (BKTree.Int.ofList xs)) = List.sort(
                                                 List.filter (fun e -> BKTree.Int.distance n e <= d) xs
                                             )
