    ...
    let testSkewBinaryRandomAccessList =
        ...
        testList "Experimental SkewBinaryRandomAccessList" [
            ...
            test "lookup length 2" {
                (((empty() |> cons "a" |> cons "b" |> lookup 0) = "b") && ((empty() |> cons "a" |> cons "b" |> lookup 1) = "a")) |> Expect.isTrue "" }

            test "lookup length 3" {
                let len3 = empty() |> cons "a" |> cons "b" |> cons "c"
                (((len3 |> lookup 0) = "c") 
                && ((len3 |> lookup 1) = "b") 
                && ((len3 |> lookup 2) = "a")) |> Expect.isTrue "" }
            ...
            test "update length 1" {
                empty() |> cons "a" |> update 0 "aa"|> lookup 0 |> Expect.equal "" "aa" } 
