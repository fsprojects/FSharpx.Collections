...
module DequeTest =
    ...
    let testDeque =
        ...
        testList "Experimental Deque" [
            ...
            test "snoc works" {
                Expect.isFalse "" (len2snoc |> isEmpty) }
            ...
            test "tail of tail of 2 empty" {
                ...
                Expect.isTrue "isEmpty" (len2snoc |> tail |> tail |> isEmpty) }

            test "initial of singleton empty" {
                Expect.isTrue "isEmpty" (len1 |> init |> isEmpty) }
            ...
            test "unsnoc works" {
                ...
                Expect.isTrue "unsnoc" ((l1 = "a") && (l2 = "b") && (l3 = "c") && (l4 = "d") && (l5 = "e") && (l6 = "f") && (isEmpty i6)) }
            ...
            test "snoc pattern discriminator" {
                ...
                Expect.isTrue "snoc" ((l2 = "b") && ((length i2) = 4)) }
            ...
            test "cons and snoc pattern discriminator" {
                ...
                Expect.isTrue "cons and snoc" ((head = "e") && (last = "b")) } 

            test "rev empty dqueue should be empty" {
                empty() |> rev |> isEmpty |> Expect.isTrue "" }
            ...
            test "tryUnsnoc on empty" {
                let q = empty()
                Expect.isNone "tryUnsnoc" <| tryUnsnoc q }

            test "tryUnsnoc on q" {
                let q = ofSeq ["a";"b";"c";"d"]
                let _, x = (tryUnsnoc q).Value 
                Expect.equal "tryUnsnoc" "d" x }

            test "tryGetHead on empty" {
                let q = empty()
                Expect.isNone "tryHead" <| tryGetHead q }

            test "tryGetHead on q" {
                let q = ofSeq ["a";"b";"c";"d"]
                Expect.equal "tryHead" "a" (tryGetHead q).Value }

            test "tryGetInit on empty" {
                let q = empty()
                Expect.isNone "tryInitial" <| tryGetInit q }
            ...
            test "tryGetLast on empty" {
                let q = empty()
                Expect.isNone "tryLast" <| tryGetLast q }

            test "tryGetLast on deque" {
                let q = ofSeq ["a";"b";"c";"d"]
                Expect.equal "tryLast" "d" (tryGetLast q).Value 
                Expect.equal "tryLast" "a" (len2 |> tryGetLast).Value 
                Expect.equal "tryLast" "a" (len2snoc |> tryGetLast).Value }

            test "tryGetTail on empty" {
                let q = empty()
                Expect.isNone "tryTail" <| tryGetTail q }

            test "tryGetTail on q" {
                let q = ofSeq ["a";"b";"c";"d"]
                Expect.equal "tryTail" "b" ((tryGetTail q).Value |> head) }
