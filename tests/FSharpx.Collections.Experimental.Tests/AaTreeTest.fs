    ...
    let testAaTree =
        testList "AaTree" [
            ...
            test "test exists returns false when item does not exist in AaTree" {
                ...
                Expect.isFalse
                <| AaTree.exists 10 tree
                <| "expect exists returns false"
            }
            ...
            test "test notExists returns true when item does not exist in AaTree" {
                ...
                Expect.isTrue
                <| AaTree.notExists 10 tree
                <| "expect notExists returns true"
            }
            ...
            test "test notExists returns false when item exists in AaTree" {
                ...
                Expect.isFalse
                <| AaTree.notExists 9 tree
                <| "expect notExists returns false"
            }

            test "test tryFind returns Some Item when Item exists in AaTree" {
                let tree = AaTree.ofList [ "hello"; "bye" ]

                Expect.equal(Some("hello"))
                <| AaTree.tryFind "hello" tree
                <| "expect tryFind returns Some Item"
            }
            ...
            test "test tryFind returns None when Item does not exist in AaTree" {
                ...
                Expect.isNone
                <| AaTree.tryFind "goodbye" tree
                <| "expect tryFind returns None"
            }
            ...
            test "test find returns Item when Item exists in AaTree" {
                ...
                Expect.equal "hello"
                <| AaTree.find "hello" tree
                <| "expect find returns item"
            }
            ...
            test "test ofList returns AaTree where all elements in list exist" {
                ...
                for i in list do
                    Expect.isTrue
                    <| AaTree.exists i tree
                    <| "expect AaTee.exists returns true on each item"
            }
         ...
         * because that is an operation where order matters. *)
            ...
            test "test inserted elements exist in AaTree" {
                ...
                let tree =
                    List.fold (fun tree el -> AaTree.insert el tree) AaTree.empty numsToInsert

                // Test that each item in the list is in the tree.
                for i in numsToInsert do
                    Expect.isTrue
                    <| AaTree.exists i tree
                    <| "expect existence of inserted elements is true"
            }
            ...
            test "test deleted elements do not exist in AaTree" {
                ...
                for i in numsToDelete do
                    Expect.isFalse
                    <| AaTree.exists i tree
                    <| "expect existence of delete elements is false"
            }
            ...
            test "test element we did not delete exists in AaTree when we delete all others" {
                ...
                Expect.isTrue
                <| AaTree.exists 3 tree
                <| "expect existence of element we did not delete is still true"
