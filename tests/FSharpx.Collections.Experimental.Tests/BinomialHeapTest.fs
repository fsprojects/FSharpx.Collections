...
module BinomialHeapTest =
    ...
    let testBinomialHeap =
        ...
        testList
            ...
            [
              ...
              test "empty list should be empty" { (BinomialHeap.empty true).IsEmpty |> Expect.isTrue "" }

              test "BinomialHeap.length of empty is 0" { (BinomialHeap.empty true).Length() |> Expect.equal "" 0 }

              test "tryGetHead on empty should return None" { (BinomialHeap.empty true).TryGetHead() |> Expect.isNone "" }

              test "BinomialHeap.tryGetTail on empty should return None" { (BinomialHeap.empty true).TryGetTail() |> Expect.isNone "" }

              test "BinomialHeap.tryGetTail on len 1 should return Some empty" {
                  let h =
                      BinomialHeap.empty true
              ...
              test "BinomialHeap.insert works" {
                  (((BinomialHeap.empty true).Insert 1).Insert 2).IsEmpty
                  |> Expect.isFalse ""
              }

              test "tryUncons empty" { (BinomialHeap.empty true).TryUncons() |> Expect.isNone "" } ]
    ...
    let propertyBinomialHeap =
        ...
        let intGens start =
            ...
            v.[1] <-
                maxBinomialHeapIntOfSeqGen
                |> Gen.filter(fun (q, l) -> l.Length >= start)

            v.[2] <-
                maxBinomialHeapIntInsertGen
                |> Gen.filter(fun (q, l) -> l.Length >= start)

            v.[3] <- minBinomialHeapIntGen

            v.[4] <-
                minBinomialHeapIntOfSeqGen
                |> Gen.filter(fun (q, l) -> l.Length >= start)

            v.[5] <-
                minBinomialHeapIntInsertGen
                |> Gen.filter(fun (q, l) -> l.Length >= start)
        ...
        testList
            "Experimental BinomialHeap property tests"
            [
              ...
              testPropertyWithConfig
                  config10k
                  "seq enumerate matches build list max"
                  (Prop.forAll(Arb.fromGen maxBinomialHeapIntGen)
                   <| fun (h, l) -> h |> List.ofSeq = l |> classifyCollect h (h.Length()))

              testPropertyWithConfig
                  config10k
                  "seq enumerate matches build list min"
                  (Prop.forAll(Arb.fromGen minBinomialHeapIntGen)
                   <| fun (h, l) -> h |> List.ofSeq = l |> classifyCollect h (h.Length()))
              ...
              testPropertyWithConfig
                  config10k
                  "tryGetHead should return 0"
                  (Prop.forAll(Arb.fromGen intGensStart2.[0])
                   <| fun (h, l) -> (h.TryGetHead().Value = l.Head) |> classifyCollect h (h.Length()))

              testPropertyWithConfig
                  config10k
                  "tryGetHead should return 1"
                  (Prop.forAll(Arb.fromGen intGensStart2.[1])
                   <| fun (h, l) -> (h.TryGetHead().Value = l.Head) |> classifyCollect h (h.Length()))

              testPropertyWithConfig
                  config10k
                  "tryGetHead should return 2"
                  (Prop.forAll(Arb.fromGen intGensStart2.[2])
                   <| fun (h, l) -> (h.TryGetHead().Value = l.Head) |> classifyCollect h (h.Length()))

              testPropertyWithConfig
                  config10k
                  "tryGetHead should return 3"
                  (Prop.forAll(Arb.fromGen intGensStart2.[3])
                   <| fun (h, l) -> (h.TryGetHead().Value = l.Head) |> classifyCollect h (h.Length()))

              testPropertyWithConfig
                  config10k
                  "tryGetHead should return 4"
                  (Prop.forAll(Arb.fromGen intGensStart2.[4])
                   <| fun (h, l) -> (h.TryGetHead().Value = l.Head) |> classifyCollect h (h.Length()))

              testPropertyWithConfig
                  config10k
                  "tryGetHead should return 5"
                  (Prop.forAll(Arb.fromGen intGensStart2.[5])
                   <| fun (h, l) -> (h.TryGetHead().Value = l.Head) |> classifyCollect h (h.Length()))
              ...
              testPropertyWithConfig
                  config10k
                  "BinomialHeap.uncons 1 element 0"
                  (Prop.forAll(Arb.fromGen intGensStart2.[0])
                   <| fun (h, l) ->
                       let x, tl = h.Uncons()
              ...
              testPropertyWithConfig
                  config10k
                  "BinomialHeap.uncons 1 element 1"
                  (Prop.forAll(Arb.fromGen intGensStart2.[1])
                   <| fun (h, l) ->
                       let x, tl = h.Uncons()
              ...
              testPropertyWithConfig
                  config10k
                  "BinomialHeap.uncons 1 element 2"
                  (Prop.forAll(Arb.fromGen intGensStart2.[2])
                   <| fun (h, l) ->
                       let x, tl = h.Uncons()
              ...
              testPropertyWithConfig
                  config10k
                  "BinomialHeap.uncons 1 element 3"
                  (Prop.forAll(Arb.fromGen intGensStart2.[3])
                   <| fun (h, l) ->
                       let x, tl = h.Uncons()
              ...
              testPropertyWithConfig
                  config10k
                  "BinomialHeap.uncons 1 element 4"
                  (Prop.forAll(Arb.fromGen intGensStart2.[4])
                   <| fun (h, l) ->
                       let x, tl = h.Uncons()
              ...
              testPropertyWithConfig
                  config10k
                  "BinomialHeap.uncons 1 element 5"
                  (Prop.forAll(Arb.fromGen intGensStart2.[5])
                   <| fun (h, l) ->
                       let x, tl = h.Uncons()
