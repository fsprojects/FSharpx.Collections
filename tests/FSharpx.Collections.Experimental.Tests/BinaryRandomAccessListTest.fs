...
module BinaryRandomAccessListTest =
    ...
    let testBinaryRandomAccessList =
        ...
        testList "Experimental BinaryRandomAccessList" [
            ...
            test "BinaryRandomAccessList.lookup BinaryRandomAccessList.length 1" { len1 |> BinaryRandomAccessList.lookup 0 |> Expect.equal "" "a" }

            test "BinaryRandomAccessList.rev empty" {
                BinaryRandomAccessList.isEmpty(BinaryRandomAccessList.empty() |> BinaryRandomAccessList.rev)
                |> Expect.isTrue ""
            }
            ...
            test "BinaryRandomAccessList.tryLookup not found" { lena |> BinaryRandomAccessList.tryLookup 10 |> Expect.isNone "" }
