        ...
        testList
            ...
            [
              ...
              test "I should be able to extract the values from the timeseries at a smaller granularity" {
                  let actual =
                      (new Timeseries<int>(startDate, granularity, size)).ToGranularity(TimeSpan.FromMinutes(5.)).AsTimeseries()
                      |> Seq.toList
              ...
              test "I should be able to extract the values from the timeseries at a larger granularity" {
                  let actual =
                      (new Timeseries<int>(startDate, TimeSpan.FromMinutes(5.), 864)).ToGranularity(TimeSpan.FromHours(1.)).AsTimeseries()
                      |> Seq.toList
