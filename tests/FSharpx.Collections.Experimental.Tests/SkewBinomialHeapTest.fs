...
module SkewBinomialHeapTest =
    ...
    let comparableAndComparable() = mixedGen() |> Arb.fromGen
    ...
    let testSkewBinomialHeap =
        ...
        testList "Experimental SkewBinomialHeap" [
            testPropertyWithConfig config10k "toSeq returns all the int elements" 
                (Prop.forAll (Arb.fromGen heapIntGen)  <|
                    fun (heap, orig) -> heap |> SkewBinomialHeap.toSeq |> Seq.toList |> List.sort = List.sort orig )

            testPropertyWithConfig config10k "toSeq returns all the string elements" 
                (Prop.forAll (Arb.fromGen heapStringGen)  <|
                    fun (heap, orig) -> heap |> SkewBinomialHeap.toSeq |> Seq.toList |> List.sort = List.sort orig ) 

            testPropertyWithConfig config10k "toSeq returns the int elements in the correct order" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) -> heap |> SkewBinomialHeap.toSeq |> Seq.toList = sortList heap orig )

            testPropertyWithConfig config10k "toSeq returns the string elements in the correct order" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> heap |> SkewBinomialHeap.toSeq |> Seq.toList = sortList heap orig )
            ...
            testPropertyWithConfig config10k "int heap isDescending returns correct value" (Prop.forAll (Arb.fromGen heapIntDescGen) <|
            ...
            testPropertyWithConfig config10k "isEmpty returns true if the int heap is empty, false otherwise"
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) -> SkewBinomialHeap.isEmpty heap = List.isEmpty orig )

            testPropertyWithConfig config10k "isEmpty returns true if the string heap is empty, false otherwise" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> SkewBinomialHeap.isEmpty heap = List.isEmpty orig )

            testPropertyWithConfig config10k "int heap count returns the number of elements" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) -> (heap |> SkewBinomialHeap.count) = List.length orig )

            testPropertyWithConfig config10k "string heap count returns the number of elements" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> (heap |> SkewBinomialHeap.count) = List.length orig )
            ...
            testPropertyWithConfig config10k "int heap head returns the first element when the heap is not empty" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                fun (heap, orig) -> 
                    heap |> SkewBinomialHeap.isEmpty |> not ==> lazy(heap |> SkewBinomialHeap.head = actualHead heap orig) )

            testPropertyWithConfig config10k "string heap head returns the first element when the heap is not empty" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> 
                         heap |> SkewBinomialHeap.isEmpty |> not ==> lazy(heap |> SkewBinomialHeap.head = actualHead heap orig) )
            ...
            testPropertyWithConfig config10k "tryHead returns Some head when the int heap is not empty"
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) -> 
            ...
            testPropertyWithConfig config10k "tryHead returns Some head when the string heap is not empty"
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> 
                        heap |> SkewBinomialHeap.isEmpty |> not ==> 
                        match SkewBinomialHeap.tryHead heap with 
            ...
            testPropertyWithConfig config10k "tail returns a heap with the first element removed when the int heap is not empty" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) ->
                        heap |> SkewBinomialHeap.isEmpty |> not ==> lazy(heap |> SkewBinomialHeap.tail |> SkewBinomialHeap.toList = actualTail heap orig) )

            testPropertyWithConfig config10k "tail returns a heap with the first element removed when the string heap is not empty" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) ->
                        heap |> SkewBinomialHeap.isEmpty |> not ==> lazy(heap |> SkewBinomialHeap.tail |> SkewBinomialHeap.toList = actualTail heap orig) )
            ...
            testPropertyWithConfig config10k "tryTail returns Some tail when the int heap is not empty" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                fun (heap, orig) -> 
                    heap |> SkewBinomialHeap.isEmpty |> not ==> 
                        lazy(
            ...
            testPropertyWithConfig config10k "tryTail returns Some tail when the string heap is not empty" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> 
                        heap |> SkewBinomialHeap.isEmpty |> not ==> 
                            lazy(
            ...
            testPropertyWithConfig config10k "uncons returns (head, tail) when the int heap is not empty" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) -> 
                        heap |> SkewBinomialHeap.isEmpty |> not ==> 
                            lazy(
            ...
            testPropertyWithConfig config10k "uncons returns (head, tail) when the string heap is not empty"
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> 
                        heap |> SkewBinomialHeap.isEmpty |> not ==> 
                            lazy(
            ...
            testPropertyWithConfig config10k "tryUncons returns Some (head, tail) when the int heap is not empty" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap, orig) -> 
                        heap |> SkewBinomialHeap.isEmpty |> not ==> 
                            lazy(
            ...
            testPropertyWithConfig config10k "tryUncons returns Some (head, tail) when the string heap is not empty" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) -> 
                        heap |> SkewBinomialHeap.isEmpty |> not ==> 
                            lazy(
            ...
            testPropertyWithConfig config10k "int heap insert always insert" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                fun (heap, orig) x ->
                    heap 
            ...
            testPropertyWithConfig config10k "string heap insert always insert" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap, orig) x ->
                        heap 
                        |> SkewBinomialHeap.insert x 
            ...
            testPropertyWithConfig config10k "Equals returns false when comparing two int heaps with the same items but different ordering" 
                (Prop.forAll (Arb.fromGen heapIntGen) <|
                    fun (heap1, orig) ->
                        let heap2 = ofList (not heap1.IsDescending) orig
                        heap1 <> heap2 && not (heap1.Equals heap2) )

            testPropertyWithConfig config10k "Equals returns false when comparing two string heaps with the same items but different ordering" 
                (Prop.forAll (Arb.fromGen heapStringGen) <|
                    fun (heap1, orig) ->
                        let heap2 = ofList (not heap1.IsDescending) orig
                        heap1 <> heap2 && not (heap1.Equals heap2) )
