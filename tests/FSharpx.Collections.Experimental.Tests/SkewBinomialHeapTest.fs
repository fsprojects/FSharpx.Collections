...
module SkewBinomialHeapTest =
    ...
    let testSkewBinomialHeap =
        ...
        testList "Experimental SkewBinomialHeap" [
            ...
            //test "tryTail returns None when the heap is empty`` () =
            //    fsCheck <| fun { Heap = heap } ->
            //        heap |> SkewBinomialHeap.isEmpty ==> (heap |> SkewBinomialHeap.tryTail |> Option.isNone)

            //test "uncons returns (head, tail) when the heap is not empty`` () =
            //    fsCheck <| fun { Heap = heap; Items = orig } -> 
            //        heap |> SkewBinomialHeap.isEmpty |> not ==> 
            //            lazy(
            ...
            //test "merge should merge if both heaps have the same ordering`` () =
            ...
            //test "merge throws when both heaps have diferent ordering`` () =
            ...
            //test "tryMerge returns Some merged heap when both heaps have the same ordering`` () =
            //    fsCheck <| fun ({ Heap = heap1; Items = orig1 }, { Heap = heap2; Items = orig2 }) ->
            //        heap1.IsDescending = heap2.IsDescending ==>
            //        lazy(
            ...
            //test "tryMerge returns None when both heaps have diferent ordering`` () =
            //    fsCheck <| fun ({ Heap = heap1; Items = orig1 }, { Heap = heap2; Items = orig2 }) ->
            //        heap1.IsDescending <> heap2.IsDescending ==> lazy(SkewBinomialHeap.tryMerge heap1 heap2 |> Option.isNone)

            //test "Cons pattern always match if the heap is not empty`` () =
            //    fsCheck <| fun { Heap = heap } ->
            //        heap |> SkewBinomialHeap.isEmpty |> not ==>
            //        match heap with
            ...
            //test "Nil pattern always match if the heap is empty`` () =
            //    fsCheck <| fun { Heap = heap } ->
            ...
            //test "Cons pattern return the same as uncons`` () =
            //    fsCheck <| fun { Heap = heap } ->
            //        heap |> SkewBinomialHeap.isEmpty |> not ==>
            //        match heap with
            ...
            //test "GetHashCode is the same for equal heaps`` () =
            ...
            //test "Equality transitivity`` () = //maybe this is too much, I guess It would be hard to write an Equals that violates this property and not the others
            //    fsCheck <| fun { Heap = heap } item ->
            //        let heap1 = heap |> SkewBinomialHeap.insert item |> SkewBinomialHeap.tail
            //        let heap2 = heap |> SkewBinomialHeap.insert item |> SkewBinomialHeap.tail
            ...
            //test "Equals returns false when comparing two heaps with the same ordering but different items`` () =
            //    fsCheck <| fun ({ Heap = heap1; Items = orig1}, { Heap = heap2; Items = orig2}) ->
            //        (heap1.IsDescending = heap2.IsDescending && orig1 <> orig2) ==> (heap1 <> heap2 && not (heap1.Equals heap2))

            //test "Equals returns false when comparing two heaps with the same items but different ordering`` () =
