...
module SkewBinomialHeapTest =
    ...
    type SkewBinomialHeapGenerators() =
        ...
        static member private EmptyOnly<'T when 'T: comparison>(d) : Gen<HeapData<'T, 'T>> =
            gen {
                let! desc = genDesc d
        ...
        static member private NonemptyOnly<'T when 'T: comparison>(d) : Gen<HeapData<'T, 'T>> =
            gen {
                let! desc = genDesc d
