...
module DequeTests =
    ...
    let testDeque =
        ...
        testList "Deque" [ test "empty dqueue should be empty" { Expect.isTrue "empty is empty" (Deque.empty |> Deque.isEmpty) }
    ...
    let propertyTestDeque =
        ...
        *)
        ...
        let dequeIntOfSeqGen = gen {
            let! n = Gen.length1thru12
            let! x = Gen.listInt n
            return ((Deque.ofSeq x), x)
        }

        let dequeIntConjGen = gen {
            let! n = Gen.length1thru12
            let! x = Gen.listInt n
