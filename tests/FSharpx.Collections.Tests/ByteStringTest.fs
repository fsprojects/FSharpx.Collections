...
module ByteStringTests =
    ...
    let spanAndSplitTests = [| "Howdy! Want to play?"B, ' 'B, 6
    ...
    let testByteString =
        testList "ByteString" [
                                ...
                                testList
                                    "test ByteString comparison should correctly return -1, 0, or 1"
                                    (comparisonTests
                                     |> List.map(fun (name, x, y, expectedResult) ->
                                         test name { BS.Compare(x, y) |> Expect.equal "comparison" expectedResult }))
                                ...
                                test "test ByteString_span correctly breaks the ByteString on the specified predicate" {
                                    ...
                                    |> Array.iter(fun (input, breakChar, breakIndex) ->
                                        ...
                                        let expected =
                                            if input.Length = breakIndex then
                                                str, ByteString.empty
                                            else
                                                BS(input, 0, breakIndex), BS(input, breakIndex, input.Length - breakIndex)

                                        Expect.equal "ByteString * ByteString" expected
                                        <| ByteString.span ((<>) breakChar) str)
                                }
                                ...
                                test "test ByteString_split correctly breaks the ByteString on the specified predicate" {
                                    ...
                                    |> Array.iter(fun (input, breakChar, breakIndex) ->
                                        ...
                                        Expect.equal "ByteString * ByteString" expected
                                        <| ByteString.split ((=) breakChar) str)
                                }
                                ...
                                test "test ByteString_split correctly breaks the ByteString on \r" {
                                    ...
                                    Expect.equal "ByteString * ByteString" expected
                                    <| ByteString.split (fun c -> c = '\r'B || c = '\n'B) str
                                }
                                ...
                                test "test drop should drop the first n items" {
                                    ...
                                    Expect.equal "ByteString" (BS(input, 7, 13))
                                    <| ByteString.skip 7 (ByteString.create input)
                                }
                                ...
                                test "test take should return an empty ArraySegment when asked to take 0" {
                                    Expect.equal "empty ByteString" ByteString.empty
                                    <| ByteString.take 0 (ByteString.create "Nothing should be taken"B)
                                }
                                ...
                                test "test take should take the first n items" {
                                    ...
                                    |> List.iter(fun x ->
                                        Expect.equal "ByteString" (BS(input, 0, x))
                                        <| ByteString.take x (ByteString.create input))
                                }

                                test "test takeWhile should return an empty ArraySegment when given an empty ArraySegment" {
                                    Expect.equal "empty ByteString" ByteString.empty
                                    <| ByteString.takeWhile ((<>) ' 'B) ByteString.empty
                                }

                                test "test takeWhile should take anything before the first space" {
                                    let input = "Hello world"B

                                    Expect.equal "ByteString" (BS(input, 0, 5))
                                    <| (ByteString.takeWhile ((<>) ' 'B) (ByteString.create input))
                                }
                                ...
                                test "test takeUntil should correctly split the input" {
                                    ...
                                    Expect.equal "ByteString" (BS(input, 0, 2))
                                    <| ByteString.takeUntil ((=) 'c'B) (ByteString.create input)
                                }

                                testProperty "test if ByteString Compare follows lexicographic order"
                                <| fun xs ys -> BS.Compare(ByteString xs, ByteString ys) = LanguagePrimitives.GenericComparison xs ys

                                testSequenced
                                <| testList "performance" [ test "Comparision should only compare relevent part of Array" {
                                                                let veryLargeArray1 = Array.init 2000 byte |> ByteString.create
                                                                let veryLargeArray2 = Array.init 2000 byte |> ByteString.create

                                                                let compareVeryLargeArray() =
                                                                    BS.Compare(veryLargeArray1, veryLargeArray2)
                                                                ...
                                                                let compareExactSameByteArray() =
                                                                    BS.Compare(theExactSameByteString1, theExactSameByteString2)

                                                                let smallByteArray1 = ByteString.take 100 veryLargeArray1
                                                                let smallByteArray2 = ByteString.take 100 veryLargeArray2

                                                                let compareSmallByteArray() =
                                                                    BS.Compare(smallByteArray1, smallByteArray2)

                                                                Expect.equal "" (compareVeryLargeArray()) (compareExactSameByteArray())

                                                                compareExactSameByteArray
                                                                |> Expect.isFasterThan "" compareVeryLargeArray

                                                                compareExactSameByteArray
                                                                |> Expect.isFasterThan "" compareSmallByteArray
