...
module NonEmptyListTests =
    ...
    let testNonEmptyList =
        ...
        testList "NonEmptyList" [
            ...
            testPropertyWithConfig config10k "NonEmptyList monad laws: left identity" (Prop.forAll(leftIdentity()) <| fun (f, a) -> ret a >>= f = f a)

            testPropertyWithConfig config10k "NonEmptyList monad laws: right identity" (Prop.forAll(neListOfInt()) <| fun x -> x >>= ret = x)
