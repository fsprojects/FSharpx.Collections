    ...
    let testSeq =
        ...
        testList
            ...
            [
              ...
              test "lift2" { Expect.equal "" [ 0; 2; 1; 3 ] (Seq.lift2 (+) [ 0; 1 ] [ 0; 2 ] |> Seq.toList) }
              ...
              test "If I tryAverage an empty seq I should get none" { Seq.empty<float> |> Seq.tryAverage |> Expect.isNone "tryAverage" }

              test "If I tryAverage a none empty seq I should get the average" { data |> Seq.tryAverage |> Expect.equal "tryAverage" (Some(5.5)) }

              test "If I tryHeadTail and I don't have a head, I should return None" {
                  Seq.empty<float> |> Seq.tryHeadTail |> Expect.isNone "tryHeadTail"
              }
              ...
              test "I should get none if try to get a index outside the seq" { Seq.tryNth 20 data |> Expect.isNone "tryNth" }
              ...
              test "I should be able to contract an empty sequence" {
                  Expect.sequenceEqual "contract" Seq.empty
                  <| Seq.contract 5 (Seq.empty)
              }
              ...
              test "I should be able to page a seq" {
                  Seq.page 0 2 data
                  |> Expect.sequenceEqual "page" (List.toSeq [ 1.; 2. ])

                  Seq.page 1 2 data
                  |> Expect.sequenceEqual "page" (List.toSeq [ 3.; 4. ])

                  Seq.page 2 2 data
                  |> Expect.sequenceEqual "page" (List.toSeq [ 5.; 6. ])
              }
